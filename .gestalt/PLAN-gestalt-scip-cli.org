...
** DONE [#B] Omit kind key entirely when value is unspecified
   Effort: 30-60 minutes

   Why: TOON rendering shows `kind: null` when the value is unspecified.
   The key should be omitted entirely in that case.

   Change:
   1. Update symbol sanitization to avoid emitting a `kind` property when
      it is missing or unspecified.
   2. Add a unit test that verifies TOON output does not contain `kind:`
      for an unspecified kind.
   ...
   Done when:
   - TOON output does not render `kind: null`
   - Tests pass

** DONE [#B] Make TOON the default CLI output format
   Effort: 30-60 minutes

   Why: TOON is now the preferred human-readable output and should be the
   default format when `--format` is not specified.

   Change:
   1. Update CLI command defaults to `toon`
   2. Update formatter default normalization to `toon`
   3. Add a unit test that verifies default output is TOON
   ...
   Done when:
   - CLI defaults to TOON for all commands
   - Tests pass
