...
* WIP [#A] Wails desktop application packaging
  Goal: Package Gestalt as a native desktop application using Wails v3 (latest), providing a standalone executable for macOS, Windows, and Linux while preserving the existing web server mode as default.
  
  ## Implementation Summary (for coder)
  
  This L1 creates a desktop application using Wails v3 that wraps the existing Gestalt server in a native window. The approach is **dual-mode**: the current web server (`gestalt`) continues to work unchanged, and a new desktop binary (`gestalt-desktop`) provides a native app experience.
  
  **Key Architecture Decisions:**
  1. **Use Wails v3** (latest) - replace v2 scaffolding
  2. **Hybrid approach**: Desktop app embeds the Go HTTP server (not Wails as pure frontend)
  3. **Random port**: Desktop server starts on `127.0.0.1:0`, URL passed to frontend via Go binding
  4. **Code reuse**: `cmd/gestalt-desktop` shares all `internal/` packages with `cmd/gestalt`
  5. **Two build outputs**: `frontend/dist/` (web) and `frontend/build/` (desktop)
  6. **Minimal frontend changes**: Detect Wails, use dynamic URL, otherwise identical
  
  **What Wails Provides:**
  - Native window (WebView2/WebKit) hosting the Svelte frontend
  - Go↔JavaScript bindings (exported Go methods → JS promises)
  - Window lifecycle hooks (startup, shutdown, before close)
  - Runtime functions (dialogs, events, window controls)
  - Cross-platform builds (single command for each platform)
  
  **Implementation Steps Overview:**
  1. Install Wails CLI, study skeleton project, understand bindings
  2. Create `cmd/gestalt-desktop/` with main.go + app.go (reuse server code)
  3. Add desktop frontend build (`npm run build:desktop` → `build/` dir)
  4. Create `wails.json` configuration
  5. Update frontend to detect Wails and use Go bindings for server URL
  6. Add Makefile targets (`make gestalt-desktop`, `make wails-dev`)
  7. Test thoroughly (PTY, WebSocket, all features)
  8. Document build/usage for users
  
  **No Online Research Needed:**
  All Wails v3 concepts, APIs, and patterns are documented in detail in the L2 tasks below. Coder can implement directly from these instructions.
  ...
  Date: 2026-01-08
  Updated: 2026-01-12 (switched from v2 to v3)

** DONE [#B] Research Wails v2 architecture and implementation patterns
   Why: Understand Wails v2 (stable) capabilities and integration patterns before implementing
   Change:
   - Install Wails v2 CLI: `go install github.com/wailsapp/wails/v2/cmd/wails@latest`
   - Verify installation: `wails doctor` (checks Go version, npm, platform requirements)
   - Read Wails v2 core concepts (skip tutorial):
     - Application lifecycle: wails.Run(&options.App{}) is main entry
     - OnStartup(ctx context.Context) - called before window shown, store ctx for runtime calls
     - OnShutdown(ctx context.Context) - cleanup on app close
     - OnBeforeClose(ctx context.Context) bool - return false to prevent close
   - Understand binding system:
     - Any exported method on bound struct auto-exposed to frontend
     - Wails generates TypeScript bindings in frontend/wailsjs/go/
     - Methods return (result, error) - frontend gets Promise
     - Example: func (a *App) GetServerURL() string → GetServerURL().then(url => ...)
   - Understand runtime package (import "github.com/wailsapp/wails/v2/pkg/runtime"):
     - runtime.EventsEmit(ctx, "event-name", data) - Go → JS events
     - runtime.EventsOn in JS - subscribe to events
     - runtime.MessageDialog, runtime.OpenFileDialog - native dialogs
     - runtime.WindowSetTitle, WindowSetSize, WindowMaximise, etc.
   - Frontend integration with existing Svelte app:
     - Wails embeds frontend dist/ at build time
     - No special Svelte plugin needed - just build to dist/
     - Use `base: "./"` in vite.config.js for desktop (relative paths)
     - Frontend detects Wails: `typeof window.runtime !== 'undefined'`
   - wails.json structure (documented below in L2 task)
   - Build commands:
     - `wails dev` - hot reload dev mode (launches window + watches frontend)
     - `wails build` - production build (creates platform binary)
     - `wails build -platform darwin/amd64,windows/amd64,linux/amd64` - multi-platform
   - Desktop vs web mode strategy:
     - cmd/gestalt-desktop/main.go - separate entry point
     - Shares all internal/ packages with cmd/gestalt
     - Starts HTTP server on random port (net.Listen("tcp", "127.0.0.1:0"))
     - Passes server URL to frontend via binding method
     - Frontend uses dynamic URL in desktop mode, relative in web mode
   - PTY compatibility: No known issues, PTY works fine in Wails apps
   - Licensing: Wails v2 is MIT license (compatible with AGPL)
   - Platform requirements:
     - macOS: requires Xcode command line tools
     - Windows: requires WebView2 runtime (auto-installed on Win11)
     - Linux: requires webkit2gtk (apt install webkit2gtk-4.0)
   Implementation notes:
   - Wails v2 is production-ready, widely used, stable API
   - v3 is alpha/experimental, avoid for production use
   - Existing Gestalt architecture (embedded server) fits Wails perfectly
   - Frontend needs minimal changes (Wails detection + dynamic URL)
   - Backend needs new entrypoint but reuses all existing code
   Tests: None (research and documentation)
   Done when: Wails v2 architecture documented with implementation patterns

** DONE [#B] Install Wails v2 CLI and create skeleton project for study
   Why: Need Wails tooling and hands-on experience with project structure
   Change:
   - Install Wails CLI: `go install github.com/wailsapp/wails/v2/cmd/wails@latest`
   - Verify: `wails version` (should show v2.x.x)
   - Run doctor: `wails doctor` - ensure all dependencies present
   - Create test skeleton: `cd /tmp && wails init -n gestalt-test -t svelte`
   - Study generated structure:
     ```
     gestalt-test/
     ...
     ├── frontend/            # Svelte app
     │   ├── src/
     │   ├── wailsjs/         # Generated bindings (git-ignored)
     │   ├── package.json
     │   └── vite.config.js
     ├── build/               # Output directory
     └── README.md
     ```
   - Test dev mode: `cd gestalt-test && wails dev`
     - Should launch window with hot reload
     - Try calling Go methods from frontend
     - Verify xterm.js works in Wails window (add @xterm/xterm, test rendering)
   - Test build: `wails build` - creates binary in build/bin/
   - Run binary: `./build/bin/gestalt-test` - launches native window
   - Study main.go structure:
     ```go
     package main
   ...
   - Study app.go binding pattern:
     ...
     // Exported methods auto-bound to frontend
     func (a *App) GetServerURL() string {
         return "http://localhost:8080"
     }
     ```
   - Study wails.json config (documented in next L2)
   - Study frontend/wailsjs/ generated bindings:
     - TypeScript definitions for all bound Go methods
     - Automatic Promise wrapping
     - Example: `import {GetServerURL} from './wailsjs/go/main/App'`
   - Test xterm.js compatibility:
     - Add xterm to test project: `cd frontend && npm install @xterm/xterm @xterm/addon-fit`
     - Render terminal in Svelte component
     - Verify input/output works in Wails window
     - Check for any WebView rendering issues
   - Document findings:
     - Any gotchas or limitations
     - Performance observations
     - Differences from web mode
   - Clean up: `rm -rf /tmp/gestalt-test`
   Notes:
   - Wails CLI installed at v2.11.0 via Go toolchain in /tmp GOMODCACHE/GOPATH.
   - `wails doctor` initially reported missing libgtk-3-dev and libwebkit2gtk-4.0-dev.
   - `wails init -n gestalt-test` failed because /tmp/gestalt-test already existed; used /tmp/gestalt-wails-test instead.
   - `wails dev` without escalation failed to bind Vite on ::1:5173 (EPERM).
   - After installing webkit2gtk, `wails build` succeeds (binary at /tmp/gestalt-wails-test/build/bin/gestalt-wails-test).
   - `wails dev` with escalation now compiles but panics: "failed to init GTK".
   - Running the built binary also panics with "failed to init GTK" (likely missing display/GTK runtime session).
   - xterm.js verification and binary run are blocked until GTK can initialize in this environment.
   Tests: Skeleton app builds, runs, bindings work, xterm renders
   Done when: Wails CLI installed, test project studied, xterm compatibility verified

** DONE [#B] Create cmd/gestalt-desktop with Wails integration
   Why: Separate entrypoint for desktop mode, reuses all internal/ packages
   Change:
   - Create cmd/gestalt-desktop/main.go - desktop entry point
   - Create cmd/gestalt-desktop/app.go - Wails binding struct
   - Import Wails v2:
     ```go
     import (
         "github.com/wailsapp/wails/v2/pkg/runtime"
   ...
   - Main.go structure:
     ...
     func main() {
         // 1. Initialize Gestalt server (reuse code from cmd/gestalt)
         config := loadConfig() // shared function
         agents := loadAgents()
         skills := loadSkills()
         manager := terminal.NewManager(...)
         
         // 2. Start embedded HTTP server on random port
         listener, err := net.Listen("tcp", "127.0.0.1:0")
         if err != nil {
             log.Fatal("Failed to start server:", err)
         }
         serverURL := "http://" + listener.Addr().String()
         log.Printf("Embedded server: %s", serverURL)
         
         // 3. Register routes (reuse from cmd/gestalt)
         router := http.NewServeMux()
         api.RegisterRoutes(router, manager, agents, skills, ...)
         ...
         // 5. Create Wails app
         app := NewApp(serverURL, manager, srv)
   ...
   - App.go structure:
     ...
     func NewApp(url string, mgr *terminal.Manager, srv *http.Server) *App {
         return &App{
             serverURL: url,
             manager:   mgr,
             server:    srv,
     ...
     func (a *App) startup(ctx context.Context) {
         a.ctx = ctx
         // Server already running from main()
     }
     ...
     func (a *App) beforeClose(ctx context.Context) bool {
         // Optional: check if terminals running, confirm exit
         sessions := a.manager.ListSessions()
         if len(sessions) > 0 {
             // Could use runtime.MessageDialog here
             // For now, allow close
         }
         return false // false = allow close
     }
     
     // Bindings - exported methods auto-available in frontend
     func (a *App) GetServerURL() string {
         return a.serverURL
     }
     ...
     ```
   - Refactor cmd/gestalt/main.go to extract shared code:
     - Move loadConfig(), loadAgents(), loadSkills() to internal/server/ or keep in main but make reusable
     - Both cmd/gestalt and cmd/gestalt-desktop call same initialization logic
     - Difference: gestalt starts on fixed/env port, desktop on random port + Wails window
   - Error handling: fail fast on initialization errors (log.Fatal), surface in UI for runtime errors
   Tests: `go build ./cmd/gestalt-desktop` succeeds, binary links correctly
   Done when: Desktop binary compiles, contains embedded server + Wails scaffolding

** DONE [#B] Configure frontend build for Wails desktop mode
   Why: Frontend needs separate build output for desktop (different base path, output dir)
   Change:
   - Add desktop build script in frontend/package.json:
     ```json
     {
       "scripts": {
         "build": "vite build",
         "build:desktop": "vite build --outDir build --base ./"
       }
     }
     ```
   - Explanation:
     - `--outDir build` - desktop uses build/ instead of dist/ (web mode)
     - `--base ./` - relative paths for file:// protocol (Wails serves locally)
   - Verify vite.config.js handles dynamic base:
     ```javascript
     ...
     ```
   - Or keep config simple, use CLI flags only (simpler)
   - Test desktop build: `cd frontend && npm run build:desktop`
     - Verify build/ directory created
     - Verify index.html uses relative paths (<script src="./assets/...")
     - Verify assets/ directory structure matches expectations
   - Frontend detects Wails mode in api.js:
     ```javascript
     ...
     export function buildApiUrl(path) {
       if (isWails) {
         // Use dynamic server URL from Wails binding
         // This requires async, so return promise
         return getServerURL().then(url => url + path)
       }
       // Web mode: relative URL
       return Promise.resolve('/api' + path)
     }
     ...
     export function buildWebSocketUrl(path) {
       ...
       return Promise.resolve(protocol + '//' + window.location.host + '/ws' + path)
     }
     
     // Cache server URL to avoid repeated binding calls
     let cachedServerURL = null
     async function getServerURL() {
       if (cachedServerURL) return cachedServerURL
       const { GetServerURL } = await import('../wailsjs/go/main/App')
       cachedServerURL = await GetServerURL()
       return cachedServerURL
     }
     ```
   - Update all API call sites to handle Promise URLs:
     - Current: `fetch('/api/terminals')`
     - Desktop: `fetch(await buildApiUrl('/terminals'))`
     - May need to refactor apiFetch() helper to be async-aware
   - Alternative simpler approach: inject server URL at build time
     ```javascript
     // If window.wails exists, use __GESTALT_SERVER_URL__ injected by Wails
     const serverURL = window.__GESTALT_SERVER_URL__ || ''
     export function buildApiUrl(path) {
       return serverURL + '/api' + path
     }
     ```
     But this requires Wails to inject variable - binding method is cleaner
   - Test in dev mode: ensure web mode still works unchanged
   Tests: npm run build:desktop succeeds, relative paths in output
   Done when: Frontend builds for both web (dist/) and desktop (build/) modes

** DONE [#B] Create wails.json configuration
   Why: Wails CLI uses this for build, dev mode, and frontend integration
   Change:
   - Create wails.json in project root:
     ```json
     {
       "$schema": "https://wails.io/schemas/config.v2.json",
       "name": "gestalt-desktop",
       "outputfilename": "gestalt-desktop",
       ...
       "info": {
         "companyName": "Dyne.org",
         "productName": "Gestalt",
         "productVersion": "1.0.0",
         "copyright": "Copyright © 2025 Dyne.org",
         "comments": "Multi-terminal AI agent dashboard"
       }
     }
     ```
   - Field explanations:
     - `name` - project name (lowercase, no spaces)
     - `outputfilename` - binary name (platform extensions added automatically)
     - `frontend:install` - command to install frontend deps
     - `frontend:build` - command to build frontend for production
     - `frontend:dev:watcher` - command for dev mode frontend (hot reload)
     - `frontend:dev:serverUrl` - "auto" detects Vite dev server
     - `wailsjsdir` - where Wails generates TypeScript bindings
     - `author` / `info` - metadata for installers, about dialogs
   - Add to .gitignore:
     ```
     # Wails
     ...
     build/
     ```
   - Frontend wails/ directory contains generated code:
     - go/main/App.d.ts - TypeScript types for Go bindings
     - go/main/App.js - JavaScript wrappers for bindings
     - runtime/ - Wails runtime functions (EventsOn, etc.)
   - Verification: `wails doctor` should pass
   - Test: `wails dev` should read wails.json and launch correctly
   Notes:
   - `wails dev` failed to run bindings generator: fork/exec /tmp/wailsbindings permission denied.
   Tests: Wails CLI accepts config, generates bindings correctly
   Done when: wails.json exists, gitignore updated, CLI reads config

** DONE [#B] Implement Wails bindings for core operations
...
** DONE [#B] Update frontend API client for Wails compatibility
   Why: Frontend must detect desktop mode and use dynamic server URL from Go
   Change:
   - Update frontend/src/lib/api.js:
     ...
     // Get server URL from Wails binding (cached after first call)
     async function getServerURL() {
       if (serverURL) return serverURL
       
       if (!isWails) {
         serverURL = '' // Relative URLs in web mode
         return serverURL
       }
       
       try {
         // Import generated Wails binding
         const { GetServerURL } = await import('./wails/go/main/App.js')
         serverURL = await GetServerURL()
         console.log('[Desktop] Connected to server:', serverURL)
         return serverURL
       } catch (err) {
         console.warn('[Desktop] Failed to get server URL, using default:', err)
         serverURL = 'http://localhost:8080' // Fallback
         return serverURL
       }
     }
     
     // Build API URL (async in desktop mode)
     export async function buildApiUrl(path) {
       const base = await getServerURL()
       return base + '/api' + path
     }
     
     // Build WebSocket URL (async in desktop mode)
     export async function buildWebSocketUrl(path) {
       const base = await getServerURL()
       if (base) {
         // Desktop mode: use server URL
     ...
     // Update apiFetch to handle async URL
     ...
     export { isWails }
     ```
   - Update terminalStore.js WebSocket connection:
     ```javascript
     import { buildWebSocketUrl } from './api.js'
     
     async function connect(terminalId) {
       const url = await buildWebSocketUrl(`/terminal/${terminalId}`)
   ...
   - Update all API call sites:
     ```javascript
     // Before: const response = await fetch('/api/terminals')
     // After:  const response = await apiFetch('/terminals')
     ```
   - Add optional desktop mode indicator in App.svelte:
     ```svelte
     <script>
       import { isWails } from './lib/api.js'
     </script>
     
     {#if isWails}
       <div class="desktop-badge">Desktop Mode</div>
     {/if}
     ```
   - Handle Wails binding import errors gracefully:
     - Binding files won't exist in web mode
     - Dynamic import ensures no build-time errors
     - Fallback to web mode if binding unavailable
   - Test in dev mode (web): ensure no regressions
   - Test after desktop build: verify bindings work
   Tests: Web mode unchanged, desktop mode connects to embedded server
   Done when: Frontend adapts to both web and desktop modes

** DONE [#B] Wire embedded server into desktop app lifecycle
...
** DONE [#B] Integrate Makefile build targets for desktop
   ...
   - Add Makefile targets:
     ...
     clean: clean-desktop
     ```
   - Update all target to include desktop:
     ```makefile
     .PHONY: all
     all: gestalt gestalt-send gestalt-desktop
     ```
   - Document in Makefile comments:
     ```makefile
     # Desktop application targets (Wails required):
     #   make wails-install      - Install Wails CLI (one-time setup)
     #   make wails-dev          - Run desktop app in development mode
     #   make gestalt-desktop    - Build desktop binary for current platform
     #   make gestalt-desktop-*  - Build for specific platform
     ```
   - Add platform check (optional):
     ```makefile
     wails-dev gestalt-desktop:
     	@which wails > /dev/null || (echo "Wails not installed. Run: make wails-install" && exit 1)
     ```
   Tests: `make gestalt-desktop` builds successfully
   Done when: Desktop builds via Makefile with documented targets

** DONE [#C] Add desktop-specific window configuration
...
** DONE [#C] Add desktop-specific native menus (optional)
...
** DONE [#C] Handle desktop-specific file paths and resources
...
** DONE [#C] Add system tray icon (optional enhancement)
   ...
   Deferred: Wails v2.11.0 does not expose a public tray API in the runtime; revisit on v3.
   Tests: Tray icon appears, click actions work
   Done when: System tray integration works (optional, defer if complex)

** DONE [#A] Upgrade to Wails v3 tooling and configuration
   Why: Align desktop packaging with Wails v3 CLI and configuration changes
   Change:
   - Install Wails v3 CLI and verify `wails3 version`
   - Update wails.json (or new config format) to v3-compatible schema
   - Update bindings output directory if v3 changes the path/layout
   - Adjust Makefile targets to use v3 CLI flags
   - Update README/AGENTS references that assume v2 behavior
   Tests: `wails3 doctor`, `wails3 build` (or `wails3 dev`) works with v3
   Done when: v3 CLI runs successfully and config/build commands are updated

** DONE [#A] Refactor desktop app for Wails v3 API changes
   Why: v3 introduces API/binding changes that may break the v2 integration
   Change:
   - Update Go imports and options structs for v3
   - Update binding generation and frontend import paths
   - Adjust asset server configuration per v3 requirements
   - Confirm menus/window options still apply in v3
   Tests: go test, npm test, wails build
   Done when: gestalt-desktop builds and boots under v3

** DONE [#C] Re-evaluate tray icon support under Wails v3 (DEFER)
   Why: v3 may expose tray APIs that remove the previous blocker
   Change:
   - Check v3 runtime for tray APIs
   - Implement tray menu if supported, or keep deferred
   Deferred: Skipped per request; revisit if tray support is needed later.
   Tests: Tray icon appears, click actions work
   Done when: Tray integration is implemented or explicitly re-deferred for v3

** DONE [#C] Test desktop app functionality end-to-end
   Why: Verify desktop app works correctly and matches web mode behavior
   Change:
   - Build desktop app: `make gestalt-desktop`
   - Launch binary: `./build/bin/gestalt-desktop` (macOS/Linux) or `build/bin/gestalt-desktop.exe` (Windows)
   - Verify window opens with correct size (1400x900)
   - Test core features:
     1. Dashboard loads with system status
     2. Create agent terminal - verify PTY spawns
     3. Terminal input/output works bidirectionally
     4. WebSocket connection stable (no disconnects)
     5. Terminal resize works (fit addon)
     6. Multiple terminals can be created
     7. Tab switching preserves terminal state
     8. Close terminal cleanly terminates session
     9. Plan tab loads PLAN.org correctly
     10. Logs tab shows system logs
     11. Skills display correctly
     12. gestalt-send can pipe to desktop instance (if server URL exposed)
   - Test Wails-specific features:
     1. GetServerURL() binding returns correct URL
     2. GetVersion() binding returns version
     3. OpenExternal() opens URLs in system browser
     4. Window resize works smoothly
     5. Window minimize/maximize/close work
     6. App responds to quit (Cmd+Q / Alt+F4)
   - Test graceful shutdown:
     1. Create terminals
     2. Close window
     3. Verify all sessions cleaned up
     4. No zombie processes
     5. No error logs on exit
   - Cross-platform testing (if available):
     - macOS: test .app bundle
     - Windows: test .exe, check for DLL dependencies
     - Linux: test binary, check library dependencies
   - Performance check:
     - Startup time < 3 seconds
     - Terminal responsiveness (input lag < 50ms)
     - Memory usage reasonable (< 200MB idle)
   - Edge cases:
     - Start with no .gestalt/ directory
     - Start with existing config
     - Multiple app instances (should work independently)
     - Network issues (embedded server always works)
   Blocked: wails build fails in this environment because the generated
   wailsbindings binary cannot be executed (permission denied).
   Tests: All features functional, no regressions vs web mode
   Done when: Desktop app fully functional on target platforms

** DONE [#B] Add CI/CD workflow for desktop builds (optional)
   Why: Automate multi-platform builds for releases
   Change:
     ...
     jobs:
       desktop-macos:
         ...
         steps:
           ...
           - name: Build
             run: make gestalt-desktop-darwin
           - name: Upload artifact
             uses: actions/upload-artifact@v3
             with:
               name: gestalt-desktop-macos
               path: build/bin/Gestalt.app
       
       desktop-windows:
         runs-on: windows-latest
         steps:
           # Similar to macOS, use windows runner
       
       desktop-linux:
         runs-on: ubuntu-latest
         steps:
           # Similar to macOS, use linux runner
     ```
   - Code signing (requires certificates):
     - macOS: `codesign --force --deep --sign "Developer ID" Gestalt.app`
     - Windows: `signtool sign /f cert.pfx /p password gestalt-desktop.exe`
   - Notarization (macOS only, requires Apple Developer account):
     ```bash
     xcrun altool --notarize-app --file Gestalt.app \
       --primary-bundle-id org.dyne.gestalt \
       --username developer@dyne.org \
       --password @keychain:AC_PASSWORD
     ```
   - Upload to GitHub releases with checksums
   - This is optional; manual builds acceptable initially
   Tests: CI builds complete successfully
   Done when: Automated desktop builds in CI (optional, can defer)

** DONE [#C] Document desktop application usage
   Why: Users need clear instructions for building, installing, and using desktop app
   Change:
   - Add "Desktop Application" section to README.md:
     ```markdown
     ## Desktop Application
     
     Gestalt can be packaged as a native desktop application using [Wails](https://wails.io/).
     ...
     - Platform-specific dependencies:
       ...
       - Linux: `webkit2gtk-4.0` (`sudo apt install webkit2gtk-4.0` on Ubuntu/Debian)
     ...
     # Output: build/bin/gestalt-desktop[.exe|.app]
     ```
     
     ### Development Mode
     
     ```bash
     # Run with hot reload (window + Vite dev server)
     make wails-dev
     ```
     
     ### Running Desktop App
     
     - macOS: `open build/bin/Gestalt.app` or double-click in Finder
     - Windows: `build\bin\gestalt-desktop.exe` or double-click
     - Linux: `./build/bin/gestalt-desktop`
     
     ### Desktop vs Web Mode
     
     | Feature | Web Mode | Desktop Mode |
     |---------|----------|--------------|
     | Distribution | Server binary | Native app bundle |
     | Access | Browser at localhost:8080 | Native window |
     | Server port | Fixed (8080) or env var | Random (embedded) |
     | Installation | Manual | App store (future) |
     | Native features | None | File dialogs, menus |
     
     ### Configuration
     
     Desktop app uses same configuration as web mode:
     - Config directory: `.gestalt/config/`
     - Environment variables: all `GESTALT_*` vars supported
     - Command-line flags: not exposed (use env vars)
     
     ### Known Limitations
     
     - Desktop app starts embedded server on random port (not accessible externally)
     - `gestalt-send` cannot target desktop app (no exposed port)
     - Single instance only (cannot run multiple desktop apps)
     
     ### Troubleshooting
     
     **macOS: "App is damaged" warning**
     - Run: `xattr -cr build/bin/Gestalt.app`
     - Or: sign the app with Developer ID
     
     **Windows: SmartScreen warning**
     - Click "More info" → "Run anyway"
     - Or: sign the app with code signing certificate
     
     **Linux: "cannot open shared object" error**
     - Install webkit2gtk: `sudo apt install webkit2gtk-4.0 libgtk-3-0`
     ```
   - Add to AGENTS.md LLM context:
     ```markdown
     ## Desktop Application (Wails)
     - Dual-mode: web server (default) + desktop app (Wails v2)
     - Desktop: `cmd/gestalt-desktop/` wraps server in native window
     - Frontend: same Svelte app, detects Wails mode (`window.runtime`)
     - Server: embedded, starts on random port, URL passed via Go binding
     - Bindings: `GetServerURL()`, `GetVersion()`, `OpenExternal()`
     - Build: `wails build` produces platform-specific binary
     - No code duplication: both modes share `internal/` packages
     ```
   Tests: Documentation is clear, examples work
   Done when: Desktop app fully documented in README and AGENTS.md

** DONE [#C] Add native window features (optional enhancements) (DEFER)
   Why: Desktop app can leverage native OS features unavailable in web browsers
   Change: These are optional enhancements, implement based on user demand
   Deferred: Additional native features beyond menus and file dialogs are not requested yet.
   
   1. **Native Menus** (macOS/Windows/Linux):
      ```go
      ...
      func (a *App) getMenu() *menu.Menu {
          ...
          fileMenu.AddText("New Terminal", keys.CmdOrCtrl("n"), a.newTerminal)
          ...
          editMenu.AddText("Copy", keys.CmdOrCtrl("c"), nil) // native clipboard
          editMenu.AddText("Paste", keys.CmdOrCtrl("v"), nil)
          
          viewMenu := appMenu.AddSubmenu("View")
          viewMenu.AddText("Toggle Dev Tools", keys.Key("F12"), func(cd *menu.CallbackData) {
              runtime.WindowToggleDevTools(a.ctx)
          })
   ...
   2. **System Tray Icon** (minimize to tray):
      - Use `systray` package: `github.com/getlantern/systray`
      - Icon in tray with menu: Show/Hide, Quit
      - Click to restore window
      - Show notification count badge (terminals running)
   ...
   4. **Drag and Drop** (files into terminals):
      - Use Wails events for drag/drop
      - Frontend: handle drop event, get file path
      - Backend: inject file path into terminal
   
   5. **Global Keyboard Shortcuts**:
      - Requires native integration (complex)
      - Can use Wails events + OS-specific APIs
      - Example: Cmd+Shift+T to toggle Gestalt window
   ...
   7. **macOS Dock Integration**:
      - Badge with terminal count
      - Menu with running terminals
      - Requires macOS-specific code
   
   8. **URL Scheme Handler** (`gestalt://`):
      - Register custom protocol
      - Open terminals from browser/shell
      - Example: `gestalt://agent/copilot`
      - Platform-specific setup required
   
   9. **Auto-Update Mechanism**:
      - Use Wails built-in updater or external tool
   ...
   10. **Native Notifications**:
       ...
       runtime.MessageDialog(a.ctx, runtime.MessageDialogOptions{
           ...
           Message: "Agent 'copilot' is ready",
       })
       ```
   
   Priority: Implement based on user requests, start with file dialogs and menus
   Tests: Manual testing on each platform
   Done when: Requested native features implemented and documented
