...
* WIP [#A] Wails desktop application packaging
  Goal: Package Gestalt as a native desktop application using Wails v2 (current stable), providing a standalone executable for macOS, Windows, and Linux while preserving the existing web server mode as default.
  
  ## Implementation Summary (for coder)
  
  This L1 creates a desktop application using Wails v2 that wraps the existing Gestalt server in a native window. The approach is **dual-mode**: the current web server (`gestalt`) continues to work unchanged, and a new desktop binary (`gestalt-desktop`) provides a native app experience.
  
  **Key Architecture Decisions:**
  1. **Use Wails v2** (stable, production-ready) - NOT v3 (alpha)
  ...
  **No Online Research Needed:**
  All Wails v2 concepts, APIs, and patterns are documented in detail in the L2 tasks below. Coder can implement directly from these instructions.
  
  Notes:
  - Dual-mode architecture: web server (default) + desktop app (opt-in build target)
  - Desktop mode: Wails v2 wraps existing Gestalt server with native window
  ...
  Date: 2026-01-08
  Updated: 2026-01-11 (switched from v3 to v2, added comprehensive implementation details)
...
** DONE [#C] Add system tray icon (optional enhancement)
   ...
   Deferred: Wails v2.11.0 does not expose a public tray API in the runtime.
   Tests: Tray icon appears, click actions work
   Done when: System tray integration works (optional, defer if complex)

** WIP [#C] Test desktop app functionality end-to-end
