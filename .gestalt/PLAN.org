* TODO [#B] CLI-specific agent configuration with TOML format and schema validation
  ...
  - JSON agent files remain supported during transition (deprecation warning)
  - Existing shell field remains but becomes optional if cli_config present
  - Migration tool provided to convert JSON to TOML (gestalt config migrate-agents)
...
** TODO [#B] Add invopop/jsonschema dependency and create schema infrastructure
...
** TODO [#B] Implement codex configuration schema from codex-config-mod.rs
...
** TODO [#C] Implement copilot configuration schema
...
** TODO [#B] Extend Agent struct to support cli_config and TOML format
   ...
   - JSON agents still load (with deprecation warning)
...
** TODO [#B] Update agent loader to support TOML files with proper validation
   ...
   2. Update internal/agent/loader.go LoadFromDir():
      - Support both *.json and *.toml file extensions
      - Try TOML parser first, fall back to JSON
      - Log deprecation warning for JSON files
   ...
   - JSON backward compatibility works
   - LoadAgentByID works for config refresh
   - All loader tests pass

** TODO [#C] Implement codex shell command generation from cli_config
...
** TODO [#C] Implement copilot shell command generation from cli_config
...
** TODO [#B] Add config refresh before terminal session start
...
** TODO [#B] Update terminal session creation to use generated shell commands
...
** TODO [#B] Create config migration CLI command
   Why: Help users migrate existing JSON configs to TOML
   
   Change:
   1. Create cmd/gestalt/migrate_agents.go:
      - Command: gestalt config migrate-agents
      - Flags: --input-dir, --output-dir, --dry-run, --force
   2. Implement migration logic:
      - Read all *.json from input dir
      - Parse as Agent structs
      - Convert to TOML format with cli_config (if cli_type recognized)
      - Write to output dir as *.toml files
      - Print summary of migrated agents
   3. Add comments to generated TOML:
      - Field descriptions from schemas
      - Migration notes if any manual steps needed
   4. Create migrate_agents_test.go:
      - Test JSON -> TOML conversion
      - Test preserving agent semantics
      - Test dry-run output
   
   Tests:
   - Migration preserves agent behavior
   - TOML output is valid and loads correctly
   - Dry-run shows what would change
   - Error handling for malformed JSON
   
   Done when:
   - gestalt config migrate-agents works
   - JSON agents convert to valid TOML
   - Comments make TOML configs self-documenting
   - Tests verify correct migration

** TODO [#C] Document new configuration system in docs/ and AGENTS.md
   Why: Users and LLM assistants need clear documentation
   
   Change:
   1. Create docs/agent-configuration.md:
      - Overview of TOML vs JSON
...
** TODO [#C] Add integration tests for full config validation workflow
...
** TODO [#C] Add CLI validation command for config files
...
** TODO [#A] Update default agent configs to use TOML with cli_config
   ...
   5. Keep JSON versions in config/ as examples of old format (with README)
...
** TODO [#B] Store agent config and hash in Temporal workflow memo
...
* WIP [#A] SCIP code intelligence integration for LLM context enhancement
...
** DONE [#B] Research SCIP format and understand SQLite schema
...
** DONE [#B] Create internal/scip package with SQLite query layer
...
** DONE [#B] Implement indexer downloader and language detection
...
** DONE [#B] Add REST API endpoints for SCIP queries
...
** DONE [#B] Create agent prompt/skill documenting SCIP API usage
...
** DONE [#C] Add indexing command to gestalt CLI
...
** DONE [#C] Optimize query performance and add caching
...
** DONE [#C] Add index freshness checking and auto-reindexing
...
** DONE [#C] Test with real codebases and document best practices
...
** DONE [#C] Add reindex recency guard and warning
...
** DONE [#B] Add comprehensive SCIP API tests using internal/scip/index.go data
...
** DONE [#B] Use system curl/wget for indexer downloads
...
** DONE [#B] Restore Go downloader with versioned asset validation
...
** DONE [#B] Embed SCIP indexer assets and extract to .gestalt/scip
...
** TODO [#C] Consider MCP server adapter (future work)
   ...
   - Decision: Defer until there's demand from external tools
   - For now: REST API sufficient for Gestalt agents
   Tests: None (future design only)
   Done when: MCP adapter design documented for future

** TODO [#B] Update documentation and add SCIP to AGENTS.md
   Why: LLMs and users need to know SCIP is available
   Change:
   - Add to AGENTS.md:
     ```markdown
     ## SCIP Code Intelligence
     - Gestalt indexes codebases using SCIP (Sourcegraph Code Intelligence Protocol)
     - Provides symbol-level understanding: functions, types, variables, etc
     - REST API: `/api/scip/symbols`, `/api/scip/symbols/:id/references`
     - Indexing: `gestalt index` command or auto-index on startup
     - Supported languages: Go, TypeScript/JavaScript, Python, Java
     - Index storage: `.gestalt/index.db` (SQLite)
     - Query examples:
       - Find function: GET /api/scip/symbols?q=ProcessOrder
       - Get definition: GET /api/scip/symbols/:id
       - Find callers: GET /api/scip/symbols/:id/references
       - List file symbols: GET /api/scip/files/:path
     - Use for: code understanding, tracing dependencies, finding usage patterns
     - Agents should query SCIP before asking about code structure
     ```
   - Add to README.md:
     ```markdown
     ## Code Intelligence (SCIP)
     ...
     ```
   - Add to agent prompt templates (optional skill):
     - Create `.gestalt/config/skills/code-intelligence/SKILL.md` (see earlier L2)
     - Or add to default agent system prompt
   ...
   Done when: SCIP fully documented in README and AGENTS.md
