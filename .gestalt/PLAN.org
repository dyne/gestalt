* TODO [#A] OpenTelemetry observability migration
  Effort: 2-3 weeks
...
** TODO [#A] Research and design OpenTelemetry integration architecture
** TODO [#B] Build custom OpenTelemetry Collector binary with ocb
** TODO [#B] Implement Collector lifecycle management in Gestalt
** TODO [#B] Integrate Collector start into Temporal workflow
** TODO [#A] Add OpenTelemetry SDK to Gestalt Go backend
** TODO [#A] Add comprehensive API instrumentation with agent-specific metrics
   Effort: 2-3 days
...
** TODO [#A] Migrate internal/logging to OpenTelemetry LogRecords
** TODO [#B] Migrate event.Bus events to OpenTelemetry LogRecords
** TODO [#B] Add distributed tracing for WebSocket connections
** TODO [#B] Integrate OpenTelemetry with Temporal workflows
** TODO [#A] Replace internal/metrics with OpenTelemetry metrics
** TODO [#B] Create frontend OTel query client
** TODO [#B] Update LogsView to use OpenTelemetry logs
** TODO [#C] Create trace viewer component
** TODO [#C] Add OpenTelemetry metrics dashboard
** TODO [#B] Implement parallel run during migration
** TODO [#B] Cut over to OpenTelemetry (remove custom systems)
** TODO [#C] Add remote backend export capability
** TODO [#C] Performance optimization and resource limits
** TODO [#C] Update documentation for OpenTelemetry observability

* WIP [#A] Fix CLI config warnings, debug logging, and log detail UX
  Goal: Remove misleading CLI config warnings, ensure debug shell logs are visible,
  and make dashboard log entries expandable with full payloads.

** WIP [#A] Remove shell+CLI config warning and improve command failure logs
   - Fix shell command parsing to honor quotes so CLI config values with spaces
     (ex: compact_prompt) are passed intact to codex/copilot.
   - Include the rendered command + args and stderr when a spawned command exits
     with an error, and downgrade the "both shell and CLI config provided" log.
   - Add tests for quoted argument parsing and CLI config with spaces.
   - Align codex CLI config keys to ConfigToml (tui.*, analytics.enabled,
     feedback.enabled, sandbox_mode) and update schema/tests/config examples.
** TODO [#B] Ensure debug logs are visible with proper filter label defaults
** TODO [#B] Expand dashboard log entries to show full payload details
...
* WIP [#A] Agent and skill loader consistency
...
** DONE [#A] Document Agent.Validate side-effects
...
** DONE [#A] Reuse prompt resolver for agent prompt validation
...
** DONE [#A] Use encoding/xml.EscapeText for SKILL XML
...
** DONE [#B] Case-fold agent name uniqueness (if desired)
...
** DONE [#B] Shared loader/path normalization helper
...
** TODO [#B] Separate validation from normalization
...
** TODO [#B] Centralize SKILL.md parsing rules
...
** TODO [#C] AgentRegistry for cache + reload behavior
...
** TODO [#C] Expand skill metadata in prompt XML
...
** TODO [#B] Extract AsyncFileLogger for session and input logs
   Effort: Medium
...
* TODO [#B] WebSocket and event stream consolidation
...
* TODO [#B] Frontend state/store simplification
  Goal: Make large UI modules smaller and more testable.
** TODO [#B] Split terminalStore into focused modules
   Effort: Medium
...
** TODO [#B] Move Dashboard data orchestration into a store
   Effort: Medium
