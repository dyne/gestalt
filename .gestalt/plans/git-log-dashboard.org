...
* DONE [#A] Frontend: Dashboard Git Log Column (replacing API metrics)
:PROPERTIES:
:Effort: 1.5d
...
** DONE [#A] Replace the “API metrics” section with “Git log” section in `Dashboard.svelte`
...
* DONE [#A] Conventional Commit Rendering + Styling
:PROPERTIES:
:Effort: 1d
...
:END:
** DONE [#A] Implement conventional commit parsing and type color mapping
...
:END:
** DONE [#B] Document `/api/git/log` in API docs
