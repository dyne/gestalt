...
* TODO [#A] Backend: Git Log Endpoint (REPR + minimal Hex adapter)
:PROPERTIES:
:Effort: 1.5d
...
:END:
** TODO [#A] Implement a `gitlog` adapter that shells out safely
...
** TODO [#A] Add `GET /api/git/log` endpoint and route registration
...
* TODO [#A] Frontend: Dashboard Git Log Column (replacing API metrics)
:PROPERTIES:
:Effort: 1.5d
...
:END:
** TODO [#A] Add API client + store plumbing for git log
