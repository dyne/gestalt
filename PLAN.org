* TODO [#B] Semantic versioning with automated releases
  ...
  - GitHub Action should detect commit types and bump version appropriately
  ...
  - UI: show version text near "Gestalt" title (small, subtle styling)
  - Tools: use semantic-release or similar for automatic version bumping and tagging
  - No manual version file edits; version is driven by git tags
  - Initial version: start from v0.1.0 if no tags exist
  Date: 2025-12-30

** TODO [#B] Setup semantic-release in GitHub Actions
   Why: Need automated version detection and git tagging based on conventional commits
   Change:
   - Create .releaserc.json configuration for semantic-release
   - Configure plugins: @semantic-release/commit-analyzer, @semantic-release/release-notes-generator, @semantic-release/github
   - Update .github/workflows/tests.yml to include release job
   - Release job: runs on push to main, analyzes commits, creates git tag, generates release notes
   - Set up GITHUB_TOKEN permissions for creating releases
   - Configure conventional commit rules: feat → minor, fix → patch, BREAKING CHANGE → major
   - Dry-run mode for testing before enabling actual releases
   Tests: Run workflow, verify it analyzes commits correctly
   Done when: GitHub Action can determine next version from commit history
...
** TODO [#C] Display version in Dashboard header
   ...
   - Add version display near "Gestalt" h1: small text, subtle color, positioned elegantly
   - Use semantic HTML: <span class="version">v{VERSION}</span>
   - Style: small font (0.7-0.8rem), muted color (#7a776f), positioned near title
   - Consider placement: "Gestalt v1.2.3" or separate line below title
   - Match existing design language from Dashboard styling
   Tests: npm run build; visually verify version displays correctly
   Done when: Version appears in Dashboard header with subtle styling

** TODO [#C] Update Makefile to integrate versioning
   Why: Local builds should support version injection
   Change:
   - Add VERSION ?= dev to Makefile
   - Update build target to pass VERSION to both Go and npm
   - Go build: add -ldflags flag with version
   - npm build: export VERSION env var before running npm run build
   - Document in comments: how to build with specific version
   - Add version target that prints current version from git tag
...
** TODO [#C] Integrate versioning into CI workflow
   Why: Automated builds should use detected version from semantic-release
   Change:
   - Update .github/workflows/tests.yml to include release steps
   - Add step to determine version: use semantic-release dry-run or git describe
   - Pass version to build steps as environment variable
   - Trigger release job only on main branch
   - Create artifacts with version in filename (optional)
   - Tag docker images with version (if docker build added later)
   Tests: Push commit with feat: or fix:, verify workflow tags correctly
   Done when: CI builds contain correct version from semantic analysis
...
** TODO [#C] Document versioning workflow
   ...
   - Document how version is injected at build time
   - Show example: "feat: add terminal history → 0.2.0", "fix: resolve connection bug → 0.1.1"
   - Link to conventional commits specification
   Tests: None (documentation only)
   Done when: README documents versioning workflow clearly

* WIP [#B] Terminal history persistence across disconnects
...
** DONE [#B] Wire existing /output endpoint into frontend on connect
...
** WIP [#C] Add optional disk persistence for terminal output
...
** TODO [#C] Load persisted history on session reconnect
...
** TODO [#C] Add session file cleanup policy
...
** TODO [#C] Add configuration options for persistence
...
** TODO [#C] Optimize file I/O for high-throughput terminals
...
** TODO [#C] Update frontend to indicate history loading state
