...
** DONE [#B] Embed Temporal server via go.temporal.io/server
   Why: Bundle Temporal server inside Gestalt without external CLI dependencies
   Change:
   - Add go.temporal.io/server dependency
   - Start embedded Temporal server when Temporal dev server is enabled
   - Store Temporal SQLite/data/logs under .gestalt/temporal
   - Replace CLI-based dev server start/stop with embedded lifecycle
   - Keep config/flags compatible with existing Temporal settings
   Tests: go test ./...; manual run to confirm server boot and client connection
   Done when: Temporal server is embedded and running without external CLI
