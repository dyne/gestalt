* TODO [#B] Prompt templating system with include directives
...
** TODO [#C] Research and decide include directive syntax
   ...
   (Architect will add findings and recommendation here after research)

** TODO [#C] Create prompt template parser module
   ...
   - Include resolution:
     ...
     - Try included filename as-is first, then with .txt appended (e.g., "common" → "common.txt")
     - Cycle detection: track include stack, return error if depth > 10 or if filename already in stack
   - Error handling:
...
** TODO [#C] Integrate template parser into terminal manager with file list logging
...
** TODO [#C] Display prompt file list in agent terminal header
...
** TODO [#B] Comprehensive test coverage for prompt templating
...
** TODO [#C] Update AGENTS.md with templating documentation
