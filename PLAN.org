* TODO [#B] gestalt-send CLI tool and agent instance management
...
** TODO [#B] Enforce unique agent names in agent loader
...
** TODO [#B] Add agent instance tracking to Manager
...
** TODO [#B] Update createTerminal API to handle duplicate agents
...
** TODO [#B] Update frontend to use agent name for tab labels
   ...
   Tests: Manual test: create agent terminal, verify tab shows agent name
   Done when: Agent terminal tabs display agent name

** TODO [#B] Handle duplicate agent in frontend gracefully
   ...
   Tests: Manual test: create agent twice, verify switch to existing tab
   Done when: Duplicate agent creation switches to existing tab

** TODO [#B] Add REST API endpoint for sending input to agent by name
...
** TODO [#B] Create gestalt-send CLI binary structure
   ...
   Tests: Build binary, verify flag parsing
   Done when: CLI binary structure exists with flag handling

** TODO [#B] Implement stdin-to-HTTP POST in gestalt-send
   ...
   Tests: Manual test: echo "test" | gestalt-send agent-name
   Done when: stdin data is sent to agent terminal successfully

** TODO [#C] Add auto-start option to gestalt-send (optional)
...
** TODO [#C] Add verbose and debug output to gestalt-send
...
** TODO [#C] Build and install gestalt-send in Makefile
...
** TODO [#C] Add shell completion for gestalt-send (optional)
...
** TODO [#C] Document gestalt-send usage and examples
...
** TODO [#C] Add integration test for end-to-end CLI workflow
...
* TODO [#B] Enhanced input UX with multiline input box and command history
  ...
  - File format: logs/input-history/{terminalId}-{timestamp}.txt (one command per line)
  - Future features enabled: search history, AI-assisted input, command review/editing, templates
  - PTY isolation: eventually all PTY input can be isolated, Ctrl+V only works in input box
  - Backward compat: existing terminals without input box continue to work (gradual migration)
...
** TODO [#B] Create input history buffer backend structure
   ...
   - Similar to OutputBuffer: maxCommands int, commands []string, mutex for thread-safety
   - Methods: Append(command string), GetAll() []string, GetRecent(n int) []string
...
** TODO [#B] Create input history persistence logger
   ...
   - File path: logs/input-history/{terminalId}-{timestamp}.txt
   - One command per line, newlines within multiline commands escaped or base64 encoded
...
** TODO [#B] Integrate input buffer into Session
   ...
   - Expose input history via Session methods: RecordInput(command string), GetInputHistory() []string
...
** TODO [#B] Add REST API endpoint for input history
   ...
   - Timestamps: track command submission time (extend InputBuffer to store timestamps)
...
** TODO [#B] Create multiline input component
...
** TODO [#B] Add history navigation to CommandInput
...
** TODO [#B] Wire CommandInput into Terminal.svelte layout
...
** TODO [#B] Send submitted commands to PTY with newline
...
** TODO [#B] Add POST endpoint to record input history
...
** TODO [#C] Optionally disable direct PTY keyboard input (future)
   Why: Eventually isolate PTY input to prevent accidental keystrokes
   Change:
...
* TODO [#B] Agent Skills integration
...
** TODO [#B] Create skill data structure and parser
...
** TODO [#B] Implement skill loader from config/skills/
...
** TODO [#B] Wire skill loader into Manager initialization
...
** TODO [#B] Extend agent profiles to reference skills
...
** TODO [#C] Generate skill metadata for agent prompts
...
** TODO [#B] Inject skill metadata into agent prompts
...
** TODO [#C] Add GET /api/skills endpoint
...
** TODO [#C] Add GET /api/skills/:name endpoint
...
** TODO [#C] Create example skills in config/skills/
...
** TODO [#C] Update frontend to display skills
...
** TODO [#C] Document Agent Skills integration
...
** TODO [#C] Add skill validation CLI command (optional)
...
** TODO [#C] Consider script execution security (future)
...
* TODO [#B] Semantic versioning with automated releases
...
** TODO [#B] Create Node.js script for version detection using semver
...
** TODO [#B] Create GitHub Actions release workflow
...
** TODO [#B] Pass version to Go backend at build time
...
** TODO [#B] Pass version to frontend at build time
...
** TODO [#C] Display version in Dashboard header
...
** TODO [#C] Update Makefile to integrate versioning
...
** TODO [#C] Integrate versioning into test workflow
...
* DONE [#B] Enable Ctrl+V paste in terminal sessions
  Goal: Make Ctrl+V paste clipboard content into xterm-backed PTY terminals without breaking Ctrl+C.
  Notes:
  - Prefer minimal, browser-safe handling (no extra dependencies).
  - Keep existing keybindings and selection behavior intact.

** Diagnose current key handling and paste flow
   Why: Identify whether Ctrl+V is blocked or not forwarded to the PTY.
   Change:
   - Review frontend xterm setup and any key handlers.
   - Trace input path from browser events to WebSocket.
   Tests: Manual reproduction in the browser terminal.
   Done when: Root cause and best fix approach are confirmed.

** Implement paste handling for Ctrl+V
   Why: Provide expected paste behavior inside PTY terminals.
   Change:
   - Add a key/paste handler that reads clipboard text and writes to terminal input.
   - Ensure Ctrl+C behavior is unchanged.
   Tests: Manual test pasting into a running shell.
   Done when: Ctrl+V reliably pastes into the terminal.
