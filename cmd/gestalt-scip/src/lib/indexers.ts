...
export function findIndexerPath(language: string, repoRoot: string): string | null {
...
export async function ensureIndexer(language: string, repoRoot: string): Promise<string> {
  const indexer = INDEXERS[language];
  if (!indexer) {
    throw new Error(`unknown indexer language: ${language}`);
  }
...
export async function runIndexer(
  ...
  const resolvedPath = indexerPath ?? (await ensureIndexer(language, repoRoot));
  const args = indexerArgs(language, outputFile, repoRoot);
  await runCommand(resolvedPath, args, repoRoot);
}

export function ensureTypeScriptProject(repoRoot: string): string | null {
  if (!repoRoot) {
    return null;
  }
...
export async function downloadBinary(url: string, destination: string): Promise<void> {
  let payload: Buffer;
