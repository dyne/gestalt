#+TITLE: Historical plan archive (legacy)
#+COMMENT: This file is kept for historical reference and is not part of the published VitePress docs.

# NOTE: Historical plan content uses "terminal" terminology; current docs/APIs use "session".
* DONE [#B] Port templating directives for runtime service discovery
  Effort: 2-3 days
  Goal: Extend prompt template parser to support {{port <service>}} directives that resolve to runtime-allocated port numbers for backend, temporal, otel, and frontend services.
  Notes:
  - Current: {{include file}} directive for composing prompts from fragments
  - New: {{port backend}}, {{port temporal}}, {{port otel}}, {{port frontend}} directives
  - Motivation: Agents need to know service ports for API calls, workflow queries, telemetry
  - All daemon ports randomized at startup to allow multiple Gestalt instances in parallel
  - Port registry: centralized runtime store of service→port mappings
  - Parser receives port resolver function/interface at construction (dependency injection)
  - Directive syntax: {{port <service-name>}} on its own line (like includes)
  - Resolution: replace directive line with port number as plain text (e.g., "8080")
  - Services: backend (REST/WS), temporal (Temporal UI), otel (collector HTTP), frontend (proxy)
  - Future-proof: registry allows adding new services without parser changes
  - Error handling: unknown service → skip directive silently (same as failed includes)
  - Architecture: prompt.Parser gets PortResolver interface, server startup populates registry
  - Testing: mock PortResolver for deterministic tests, integration test with real server ports
  Date: 2026-01-25
  Priority: [#B] - enables agents to discover services dynamically in multi-instance deployments

** [#B] Design port registry and resolver interface
   Effort: 2-3 hours
   Why: Need clean abstraction for service→port lookups that parser and server can share
   Change:
   - Create internal/ports package with PortRegistry and PortResolver interfaces
   - PortRegistry: mutable map with Set(service, port), Get(service) methods
   - Thread-safe with sync.RWMutex for concurrent reads from prompt rendering
   - PortResolver: read-only interface with single Get(service) (int, bool) method
   - Parser depends only on PortResolver (narrow interface, testable)
   - Service names: "backend", "frontend", "temporal", "otel" (lowercase, extensible)
   - Registry singleton or passed via server context (decide in implementation)
   - Document service naming conventions: lowercase, no spaces, descriptive
   Tests: Unit tests for registry Set/Get, concurrent access, unknown services
   Done when: Port registry exists with clean resolver interface for parser

** [#B] Extend prompt parser to accept PortResolver
   Effort: 3-4 hours
   Why: Parser needs port information at render time to resolve {{port}} directives
   Change:
   - Add PortResolver field to Parser struct (optional, can be nil)
   - Update NewParser signature: NewParser(promptFS, promptDir, includeRoot, resolver PortResolver)
   - Existing callers without port support: pass nil resolver (backward compat)
   - In renderFile loop: detect {{port <service>}} directives alongside {{include}}
   - Add parsePortDirective(line string) (service string, ok bool) helper
   - Directive syntax: "{{port backend}}" → extract "backend" as service name
   - Validation: service name must be single word (no spaces), 1-32 chars
   - If resolver is nil: skip port directives silently (no error)
   - If resolver is non-nil but service unknown: skip directive silently
   Tests: Unit tests for parsePortDirective, various formats, edge cases
   Done when: Parser recognizes port directives and parses service names

** [#B] Implement port directive resolution in render loop
   Effort: 2-3 hours
   Why: Replace {{port}} directives with actual port numbers during template rendering
   Change:
   - In renderFile line-processing loop (after include check):
   - Check if parsePortDirective(line) succeeds
   - If yes and resolver non-nil: call resolver.Get(service)
   - If port found: write port as string to output (fmt.Sprintf("%d", port))
   - If port not found or resolver nil: skip directive line entirely (silent)
   - Preserve line ordering: port replacement happens inline, not appended
   - Add newline after port number to match line-based parsing
   - Update RenderResult.Files: port directives don't add files (not includes)
   Tests: Unit tests with mock resolver, verify port numbers appear in output
   Done when: {{port backend}} in template renders as "8080" (from mock resolver)

** [#B] Populate port registry at server startup
   Effort: 3-4 hours
   Why: Server knows all allocated ports after daemon startup, must register them
   Change:
   - Create global or context-scoped PortRegistry in cmd/gestalt/server_command.go
   - After backend listener created: registry.Set("backend", backendPort)
   - After frontend server created: registry.Set("frontend", cfg.FrontendPort)
   - After temporal dev server started: registry.Set("temporal", temporalUIPort)
   - After otel collector started: registry.Set("otel", collectorHTTPPort)
   - Extract port from listener/config/daemon structs (varies by service)
   - Temporal UI port: read from temporalDevServer struct or config
   - OTel port: read from collector options or running process
   - Pass registry to app.Build or Manager creation (decide on wiring)
   - Manager/Session needs registry to pass to prompt parser
   Tests: Integration test verifying registry populated after startup
   Done when: All service ports registered and available for lookup

** [#B] Wire port resolver into prompt rendering flow
   Effort: 2-3 hours
   Why: Manager/Session prompt rendering must use port-aware parser
   Change:
   - Update Manager to hold PortResolver reference (passed via ManagerOptions)
   - Update Manager.readPromptFile to create Parser with resolver
   - Current: NewParser(m.promptFS, m.promptDir, m.includeRoot)
   - New: NewParser(m.promptFS, m.promptDir, m.includeRoot, m.portResolver)
   - Session prompt injection receives resolved ports in rendered content
   - Add PortResolver to ManagerOptions struct (optional field)
   - Nil resolver is valid (ports not available, directives skipped)
   Tests: Manager test with mock resolver, verify ports in prompt output
   Done when: Agent prompts contain actual port numbers when {{port}} used

** [#C] Add tests for port directive rendering
   Effort: 2-3 hours
   Why: Ensure directive parsing and resolution work correctly in all scenarios
   Change:
   - Add test template: "API: {{port backend}}\nTemporal: {{port temporal}}"
   - Mock resolver returns: backend=8080, temporal=7233, otel=4318
   - Verify rendered output contains port numbers as plain text
   - Test unknown service: {{port unknown}} → directive line skipped
   - Test nil resolver: all directives skipped silently
   - Test mixed template: includes + port directives in same file
   - Test nested templates: port directives in included files work
   - Add integration test: server startup → agent prompt → verify real ports
   Tests: All test scenarios pass, parser_test.go and integration tests
   Done when: Port directive feature has comprehensive test coverage

** [#C] Document port directive syntax and available services
   Effort: 1-2 hours
   Why: Users and LLMs need to know how to use port directives in prompts
   Change:
   - Update AGENTS.md prompt templating section with {{port}} directive
   - Document syntax: "{{port <service>}}" on its own line
   - List available services: backend, frontend, temporal, otel
   - Explain use case: "Agents can query services using dynamic ports"
   - Add example template showing port usage for API calls
   - Document behavior: unknown services skipped silently
   - Document multi-instance: ports randomized, agents get correct local ports
   - Note: port directives only work when services are running
   Tests: None (documentation only)
   Done when: Port directive feature documented in AGENTS.md

** [#C] Add example prompt using port directives
   Effort: 1 hour
   Why: Demonstrate real-world usage of port directives in agent prompts
   Change:
   - Create example prompt: config/prompts/service-discovery.tmpl
   - Content: list service ports with {{port <service>}} directives on their own lines
   - Example guidance: build URLs using the rendered port numbers (e.g., http://localhost:<backend-port>/api/status)
   - Update example agent config to include service-discovery prompt
   - Verify agent receives correct ports when prompt is rendered
   Tests: Manual test with example agent, verify ports in terminal
   Done when: Example prompt demonstrates port directive usage

** [#C] Consider adding port directive to skill XML (future)
   Effort: 2-3 hours (future work)
   Why: Skills may also need service port information for tool definitions
   Change:
   - Skill XML generation could benefit from port substitution
   - Example: <api>http://localhost:{{port backend}}/api/skills</api>
   - Requires: skill renderer to use same port resolver as prompt parser
   - This is future enhancement; prompts are sufficient for initial implementation
   - Document design consideration in this L2 for future reference
   Tests: None (future work)
   Done when: Design documented for future skill port support (optional)

** TODO [#C] Handle edge case: daemon not started (port unavailable)
   Effort: 1-2 hours
   Why: Port directives should fail gracefully when services aren't running
   Change:
   - Current design: unknown service → skip directive silently
   - Consider: add logging when port requested but service not registered
   - Log at debug level: "port directive for 'temporal' skipped, service not available"
   - Helps users debug why agent prompt doesn't contain expected ports
   - Alternative: add metadata to rendered result indicating skipped directives
   - Decision: silent skip is consistent with include behavior (decided above)
   Tests: Test prompt with {{port temporal}} when Temporal not running
   Done when: Graceful handling of unavailable services documented and tested

* TODO [#B] Add Web Speech API voice dictation to agent input
  Effort: 2-3 days
  Goal: Integrate browser Web Speech API (SpeechRecognition) for voice-to-text dictation in agent command input boxes.
  Notes:
  - API: Web Speech API SpeechRecognition (built into modern browsers, no external service)
  - UX: microphone button in/beside agent input box
  - Behavior: click to start/stop listening; transcribed text appends to input field
  - Visual feedback: recording indicator (red dot/pulsing mic icon)
  - Error handling: permission denied, API not supported, recognition errors
  - Browser support: Chrome/Edge (webkit), Firefox (limited), Safari (webkit)
  - No backend changes needed (purely frontend feature)
  - Accessibility: keyboard shortcut for voice input (e.g., Ctrl+Shift+V)
  - Settings: optional continuous recognition vs single phrase
  - Language: default browser language; optionally configurable
  Date: 2026-01-25
  Priority: [#B] - nice-to-have UX improvement for hands-free interaction

** TODO [#B] Research Web Speech API and browser support
   Effort: 2-3 hours
   Why: Understand API capabilities, limitations, and cross-browser compatibility
   Change:
   - Read MDN docs: https://developer.mozilla.org/en-US/docs/Web/API/Web_Speech_API
   - Understand SpeechRecognition interface: start(), stop(), onresult, onerror
   - Document browser support: Chrome/Edge (full), Firefox (partial), Safari (webkit prefix)
   - Identify key properties: continuous, interimResults, lang
   - Test basic example in browser console to verify API availability
   - Document fallback strategy for unsupported browsers
   - Note: requires HTTPS or localhost (security requirement)
   Tests: None (research only)
   Done when: API capabilities and browser support documented

** TODO [#B] Create voice input Svelte component
   Effort: 4-6 hours
   Why: Reusable component for microphone button + speech recognition
   Change:
   - Create frontend/src/components/VoiceInput.svelte
   - Props: onTranscript (callback with text), continuous (boolean), lang (string)
   - State: isListening, isSupported, error
   - Initialize SpeechRecognition with webkit prefix fallback
   - Implement start/stop methods
   - Handle onresult event: extract transcript, call onTranscript callback
   - Handle onerror event: set error state, emit error event
   - Handle onend event: reset listening state
   - Visual: microphone icon button (changes color when active)
   - Pulsing animation during recognition
   - Tooltip showing status (click to start, listening, processing)
   - Emit custom events: start, stop, transcript, error
   Tests: Manual testing in supported browsers; verify transcript callback
   Done when: Component captures voice input and emits transcript

** TODO [#B] Integrate voice input into CommandInput component
   Effort: 2-3 hours
   Why: Add voice input capability to agent input boxes
   Change:
   - Update frontend/src/components/CommandInput.svelte
   - Add VoiceInput component beside text input field
   - Position microphone button at right edge of input box
   - On transcript callback: append text to input value
   - Add space between existing text and new transcript
   - Auto-focus input field after transcript
   - Optional: show "Voice input not supported" if API unavailable
   - Style: microphone button matches input field height
   - Mobile: ensure button is touch-friendly (min 44px tap target)
   Tests: Manual test voice input in Dashboard agent command boxes
   Done when: Voice input works in agent input fields

** TODO [#C] Add keyboard shortcut for voice input
   Effort: 1-2 hours
   Why: Accessibility and power user efficiency
   Change:
   - Add keyboard listener in CommandInput (e.g., Ctrl+Shift+V or Cmd+Shift+V)
   - Toggle voice input on shortcut press
   - Show shortcut hint in tooltip or help text
   - Prevent conflict with browser paste shortcut
   - Document shortcut in UI or README
   Tests: Verify keyboard shortcut triggers voice input
   Done when: Keyboard shortcut toggles voice input

** TODO [#C] Add voice input settings and controls
   Effort: 2-3 hours
   Why: User preferences for language, continuous recognition, etc.
   Change:
   - Add settings panel for voice input (modal or inline)
   - Setting: language selection (dropdown with common languages)
   - Setting: continuous recognition vs single phrase mode
   - Setting: interim results (show partial transcripts)
   - Store settings in localStorage
   - Apply settings to SpeechRecognition instance
   - Default: browser language, single phrase mode, interim results off
   - Settings icon beside microphone button or in app settings
   Tests: Manual test settings persistence and application
   Done when: Users can customize voice input behavior

** TODO [#C] Improve error handling and user feedback
   Effort: 2-3 hours
   Why: Clear feedback when voice input fails or isn't available
   Change:
   - Check for SpeechRecognition support on mount
   - Show clear message if API not available (browser not supported)
   - Handle permission denied: show prompt to allow microphone access
   - Handle no-speech error: show "No speech detected" message
   - Handle network error: show "Network error, try again" message
   - Handle aborted error: silent (user stopped intentionally)
   - Add retry button for transient errors
   - Log errors to console for debugging
   - Optional: show error toast for critical failures
   Tests: Manual test error scenarios; verify user-friendly messages
   Done when: All error states have clear user feedback

** TODO [#C] Add visual recording indicator
   Effort: 1-2 hours
   Why: Users need clear feedback that voice input is active
   Change:
   - Add red dot or pulsing animation to microphone button when recording
   - Optional: show waveform or audio level indicator
   - Add "Listening..." text near input field
   - Change button color/icon when active (red microphone)
   - Add CSS animation for pulsing effect
   - Ensure indicator is visible but not distracting
   Tests: Manual visual testing; verify indicator shows during recording
   Done when: Recording state is visually obvious

** TODO [#C] Test cross-browser compatibility
   Effort: 2-3 hours
   Why: Ensure voice input works across supported browsers
   Change:
   - Test in Chrome/Chromium (full support)
   - Test in Edge (Chromium-based, full support)
   - Test in Firefox (limited support, may need polyfill)
   - Test in Safari (webkit prefix required)
   - Document browser-specific quirks and limitations
   - Add polyfill or fallback if needed
   - Update README with browser support matrix
   Tests: Manual testing in each browser; verify functionality
   Done when: Voice input works in all major browsers or gracefully degrades

** TODO [#C] Add documentation for voice input feature
   Effort: 1 hour
   Why: Users need to know voice input is available and how to use it
   Change:
   - Add "Voice Input" section to README.md
   - Document browser support and requirements (HTTPS/localhost)
   - Document keyboard shortcut
   - Document settings (language, continuous mode)
   - Add screenshot or GIF showing voice input in action
   - Document troubleshooting (microphone permission, browser support)
   Tests: None (documentation only)
   Done when: Voice input feature is documented in README

** TODO [#C] Optimize for mobile devices
   Effort: 2-3 hours
   Why: Mobile users benefit from voice input even more than desktop
   Change:
   - Ensure microphone button is touch-friendly (min 44px)
   - Test on iOS Safari (webkit prefix)
   - Test on Android Chrome (full support)
   - Handle on-screen keyboard interaction (input focus)
   - Consider fullscreen voice input mode for mobile
   - Add haptic feedback on start/stop (if available)
   - Test with mobile microphone permissions
   Tests: Manual testing on iOS and Android devices
   Done when: Voice input works smoothly on mobile browsers

* DONE [#B] Strip ANSI escape sequences and noise from log context fields
  Effort: 1-2 days
  Goal: Clean up log output by filtering ANSI escape sequences, control codes, and repeated character sequences from terminal output stored in log context fields.
  Notes:
  - Problem: Terminal output in logs contains raw escape sequences from CLI agents (colors, animations, cursor positioning)
  - Example: output_tail field shows "\x1b[0m\x1b[49m\x1b[K\x1b[71;2H..." making logs unreadable
  - Affects: output_tail (terminal manager), context (Temporal bell), stderr (session factory)
  - Solution: Filter at write time before adding to log context fields
  - Keep: ASCII text, UTF-8 text, newlines, spaces, tabs
  - Remove: ANSI escape sequences, control codes, repeated character sequences (e.g., ─────)
  - Performance: minimal overhead (regex compile once, apply on small strings before logging)
  - Testing: verify filtering works at write time, and filtered output appears in WebSocket streams
  Date: 2026-01-25
  Priority: [#B] - improves log readability and debugging experience

** [#B] Create ANSI/control code filter utility
   Effort: 3-4 hours
   Why: Need reusable function to strip escape sequences and noise from strings
   Change:
   - Create internal/terminal/textfilter.go with StripANSI(s string) string function
   - Strip ANSI escape sequences: \x1b[...m (colors), \x1b[...H (cursor), \x1b[...J (clear), etc
   - Pattern: \x1b\[[0-9;]*[A-Za-z] covers most ANSI CSI sequences
   - Strip other control codes: \x00-\x1f except \t (tab), \n (newline), \r (return)
   - Add StripRepeatedChars(s string, minLen int) string for repeated sequences
   - Pattern: (.)\1{N,} where N = minLen-1 (e.g., ─── with minLen=3)
   - Collapse to single instance: "────────" → "─"
   - Note: implemented repeated-char collapse via rune scan (Go regexp lacks backrefs)
   - Combine: FilterTerminalOutput(s string) string calls both filters
   - Use compiled regexps (compile once in init or var)
   - Handle edge cases: empty strings, already-clean strings, UTF-8 characters
   - Document filter behavior with examples in function comments
   Tests: Unit tests in textfilter_test.go with various ANSI sequences, repeated chars, mixed content
   Done when: Filter functions work correctly and efficiently

** [#B] Apply filter to output_tail in terminal manager
   Effort: 1-2 hours
   Why: output_tail is the primary source of noisy terminal output in logs
   Change:
   - Update internal/terminal/manager.go renderOutputTail function
   - After joining lines, before truncation: apply FilterTerminalOutput
   - Current flow: join lines → truncate to maxBytes → return
   - New flow: join lines → FilterTerminalOutput → truncate to maxBytes → return
   - Filtering happens before truncation to ensure clean text within byte limit
   - Update call sites in manager_lifecycle.go and manager_prompt.go (no changes needed if renderOutputTail is updated)
   - Add debug logging to show before/after lengths if filtering reduces size significantly
   Tests: Update manager tests to verify filtered output in log context fields
   Done when: output_tail fields in logs are ANSI-free

** [#B] Apply filter to bell context in Temporal activities
   Effort: 1 hour
   Why: Bell context contains terminal output with ANSI sequences
   Change:
   - Update internal/temporal/activities/session_activities.go RecordBellActivity
   - Before logging, filter contextText parameter: contextText = textfilter.FilterTerminalOutput(contextText)
   - Import internal/terminal (or extract textfilter to internal/textfilter if better separation)
   - Keep filter call early in function (before any logging/storage)
   Tests: Update session_activities_test.go to verify filtered context in logs
   Done when: Bell context fields are ANSI-free

** [#B] Apply filter to stderr in session factory
   Effort: 30 minutes
   Why: stderr may contain ANSI output from shell/command startup
   Change:
   - Update internal/terminal/session_factory.go error logging
   - Find where stderr is logged: fields["stderr"] = stderr
   - Apply filter: fields["stderr"] = textfilter.FilterTerminalOutput(stderr)
   - Same pattern as other context fields
   Tests: Add test case with ANSI in stderr, verify filtered output
   Done when: stderr fields in logs are ANSI-free

** [#B] Add integration test for WebSocket log stream filtering
   Effort: 2-3 hours
   Why: Verify filtered logs appear correctly in real-time WebSocket streams
   Change:
   - Create test in internal/api/ws_logs_test.go (or existing test file)
   - Scenario: create terminal, trigger error with output_tail, connect to /ws/logs
   - Mock terminal output with ANSI sequences
   - Verify WebSocket message contains filtered text (no escape sequences)
   - Test flow: terminal output → log context → WebSocket → client receives clean text
   - Also test via GET /api/logs to verify REST endpoint returns filtered logs
   - Add test case for bell context via Temporal workflow
   Tests: Integration test passes; WebSocket and REST return filtered logs
   Done when: End-to-end filtering verified through WebSocket and REST APIs

** [#C] Add performance benchmarks for text filtering
   Effort: 1-2 hours
   Why: Ensure filtering doesn't impact logging performance
   Change:
   - Create textfilter_bench_test.go with benchmarks
   - Benchmark FilterTerminalOutput with various input sizes (100B, 1KB, 10KB)
   - Benchmark ANSI-heavy vs clean text (should short-circuit on clean text)
   - Benchmark repeated character detection
   - Target: < 1µs for typical log context strings (~2KB)
   - Document results in benchmark file comments
   Tests: Run benchmarks; go test -bench=. ./internal/terminal/
   Done when: Benchmarks show acceptable performance (< 1ms for 10KB)

** [#C] Update documentation for filtered logs
   Effort: 30 minutes
   Why: Users should know logs are cleaned for readability
   Change:
   - Add note to AGENTS.md under logging section
   - Document: "Terminal output in log context fields is automatically cleaned"
   - Explain what's removed: ANSI escape sequences, control codes, repeated characters
   - Explain what's kept: text, newlines, spaces, tabs, UTF-8 characters
   - Mention benefit: improved log readability, easier debugging
   Tests: None (documentation only)
   Done when: Filtering behavior documented in AGENTS.md

** [#C] Add tests for real-world ANSI log samples
   Effort: 1 hour
   Why: Ensure filtering handles observed log output with dense ANSI/control sequences
   Change:
   - Add test coverage using provided real-world log samples
   - Validate filtered output has no ANSI/control sequences and collapses repeated characters
   - Ensure key text fragments remain readable
   Tests: Extend textfilter_test.go or integration tests
   Done when: Real-world samples are covered by tests and pass

* TODO [#A] Diagnose and fix high CPU usage in idle gestalt
  Effort: 1 hour (quick win!)
  Goal: Identify and eliminate the cause of ~40% CPU usage when gestalt is idle with no active terminals or agents doing work.
  Notes:
  - Problem: gestalt process uses ~16% CPU constantly (62:35 CPU time over 6.5 hours) even when idle
  - Root cause: busy-wait loop in session.go PTY read loop (lines 564-568)
  - The select/default pattern prevented pty.Read() from blocking, causing tight polling loop
  - Fix: removed select statement, let pty.Read() block naturally (PTY reads are blocking by default)
  - Testing: all terminal tests pass after fix
  - CPU usage should drop from ~40% to <1% when idle
  - Future: add CPU regression test and pprof infrastructure for easier debugging
  Date: 2026-01-24
  Priority: [#A] - performance regression blocking production use
  Resolution: Fixed in commit 01b64ce

** DONE [#A] Identify root cause via code inspection
   Effort: 2-3 hours
   Why: Can't fix what we can't measure; need runtime profiling data to identify hot paths
   Change:
   - Add pprof HTTP endpoint to gestalt server
   - Register net/http/pprof handlers on debug endpoint (e.g., /debug/pprof/)
   - Make pprof endpoint optional: GESTALT_PPROF_ENABLED (default: false in production, true in dev)
   - Add --pprof flag to enable profiling endpoints
   - Document pprof endpoints: /debug/pprof/profile (30s CPU profile), /debug/pprof/goroutine (goroutine dump)
   - Add helper script: scripts/profile-cpu.sh that fetches profile and opens it
   - Test profiling on idle system: verify we can capture and analyze profiles
   Tests: Start gestalt with --pprof, curl /debug/pprof/profile, verify profile downloads
   Done when: Can capture CPU profiles from running gestalt process

** DONE [#A] Capture and analyze baseline CPU profile
   Effort: 1-2 hours
   Why: Need to see what functions are consuming CPU in idle state
   Change:
   - Start gestalt with pprof enabled and let it idle with 2 shell terminals open
   - Capture 30s CPU profile: go tool pprof http://localhost:8080/debug/pprof/profile?seconds=30
   - Analyze with pprof interactive mode: top20, list (hot functions)
   - Check for: tight loops, excessive channel operations, lock contention, GC pressure
   - Generate flame graph: go tool pprof -http=:8081 profile.pb.gz
   - Document findings in this L2 task (edit PLAN.org with analysis results)
   - Common culprits to check:
     - Event bus publish/subscribe in hot path
     - OTel SDK metric recording in tight loop
     - Terminal output buffer management
     - WebSocket read/write loops without blocking
     - Ticker/timer goroutines firing too frequently
   Tests: Profile analysis documented with top CPU consumers identified
   Done when: Root cause identified with concrete function names and call stacks

** DONE [#A] Fix identified CPU hotspot
   Effort: varies (2 hours - 1 day depending on cause)
   Why: Eliminate the busy-wait or excessive polling that's burning CPU
   Change:
   - Based on profile analysis, implement targeted fix
   - Common fixes:
     - Add blocking reads instead of polling (select with timeout, not busy loop)
     - Reduce event bus publish frequency (batch events, debounce)
     - Fix goroutine that's spinning (add proper channel blocking)
     - Reduce metric recording frequency (sample instead of every event)
     - Add sleep/delay in tight loop (if loop is intentional)
   - Specific areas to check based on recent OTel changes:
     - internal/otel/middleware.go: check if metrics recorded synchronously on every request
     - internal/otel/sdk.go: check if tracer/meter doing background work
     - internal/event/*.go: check if Bus publish is O(n) subscribers without optimization
     - internal/terminal/session.go: check broadcastLoop for busy-wait
   - Add comments explaining the fix
   Tests: Unit tests for fixed component; go test ./...
   Done when: Fix implemented, tests pass, ready for verification

** WIP [#B] Verify CPU usage fix with profiling
   Effort: 1 hour
   Why: Confirm fix actually reduced CPU usage to acceptable levels
   Change:
   - Rebuild gestalt with fix applied
   - Run with same idle scenario: 2 shell terminals, no activity
   - Capture new CPU profile after 10 minutes runtime
   - Compare before/after profiles: verify hotspot is gone
   - Monitor actual CPU usage with top/htop: should be <5% when idle
   - If not fixed: return to analysis step, find next hotspot
   - Document before/after metrics in PLAN.org
   Tests: CPU usage < 5% when idle, profile shows no busy loops
   Done when: Idle CPU usage reduced to acceptable baseline

** TODO [#C] Add mock agent for testing (prevent real CLI dependencies)
   Effort: 2-3 hours
   Why: Tests must not require real codex/copilot accounts; need deterministic mock
   Change:
   - Create internal/terminal/mock_agent.sh shell script that simulates agent behavior
   - Script echoes canned responses, accepts commands, doesn't invoke real LLM CLIs
   - Add test agent config: config/agents/mock.toml using mock_agent.sh as shell
   - Script features:
     - Echoes startup message with onair_string
     - Accepts prompt injection without errors
     - Responds to simple commands with deterministic output
     - Optionally: simulate bell signal after N commands
   - Update tests to use mock agent instead of real agents
   - Document mock agent in TESTING.md
   Tests: Tests using mock agent run without real CLI dependencies
   Done when: Test suite uses mock agent exclusively, no codex/copilot required

** TODO [#C] Add CPU regression test
   Effort: 2-3 hours
   Why: Prevent future regressions; catch busy-wait issues in CI
   Change:
   - Add integration test: internal/terminal/cpu_test.go
   - Test creates 2 mock agent terminals, lets them idle for 10s
   - Measure goroutine count before/after: should be stable (no leaks)
   - Measure CPU time used during idle: should be < 1% of wall time
   - Use runtime.ReadMemStats, runtime.NumGoroutine for metrics
   - Optional: use runtime/pprof to verify no hot paths during idle
   - Test runs in CI but with generous thresholds (CI may be noisy)
   - Document expected idle behavior in test
   Tests: CPU regression test passes on idle system
   Done when: Test catches excessive CPU usage in idle state

** TODO [#C] Document profiling workflow for future debugging
   Effort: 1 hour
   Why: Make it easy to profile performance issues in the future
   Change:
   - Add "Performance Profiling" section to TESTING.md or new docs/profiling.md
   - Document how to enable pprof endpoints
   - Document how to capture CPU profiles: go tool pprof commands
   - Document how to analyze profiles: top, list, web, flame graphs
   - Document how to capture memory profiles: /debug/pprof/heap
   - Document how to capture goroutine dumps: /debug/pprof/goroutine
   - Add troubleshooting section: common issues and their profile signatures
   - Link to Go pprof documentation and blog posts
   Tests: None (documentation only)
   Done when: Profiling workflow documented for contributors

** TODO [#C] Review OTel middleware for synchronous overhead
   Effort: 1-2 hours
   Why: API instrumentation added today might be recording metrics synchronously on hot path
   Change:
   - Review internal/otel/middleware.go for potential performance issues
   - Check if metrics recorded synchronously for every request (should be async/batched)
   - Check if tracer spans created eagerly (should be lazy or sampled)
   - Compare OTel SDK overhead to baseline: measure request latency before/after instrumentation
   - If overhead > 5%, investigate SDK configuration (batch size, sampling rate)
   - Document findings and optimizations
   Tests: Benchmark middleware overhead; verify request latency unchanged
   Done when: OTel middleware overhead is negligible (< 5% request latency)

* DONE [#A] OpenTelemetry observability migration
  Effort: 2-3 weeks
  Goal: Replace Gestalt's custom event/logging system with OpenTelemetry for industry-standard observability, distributed tracing, and metrics.
  Notes:
  - Current state: internal/event.Bus[T] with custom event types, internal/logging package, ad-hoc metrics
  - New approach: OpenTelemetry SDK for events/logs/traces, custom Collector binary, localhost OTLP
  - Build custom otelcol-gestalt using OpenTelemetry Collector Builder (ocb) with minimal components
  - Collector lifecycle: start at Gestalt launch (before Temporal dev server), stop on exit
  - Temporal integration: record Collector start in workflow, ensure clean shutdown coordination
  - Event format: map Gestalt events to OTel LogRecords with structured attributes (native OTel primitives)
  - Frontend: query Collector's OTLP HTTP endpoint for logs, default to INFO level, expandable details
  - Trace context: propagate through WebSocket connections, Temporal workflows, terminal sessions
  - Metrics: expose as OTel metrics (terminal count, event rates, workflow states)
  - API instrumentation: comprehensive metrics for all REST endpoints with agent identification
  - API metrics: request count, duration, size, errors, active requests - all by endpoint, agent, category
  - Agent profiling: track which agents call which APIs, frequency, performance, errors
  - Dashboard queries: most called APIs, slowest endpoints, agent efficiency, error rates by category
  - Migration strategy: parallel run custom events + OTel during transition, then cut over
  - Collector components needed: otlpreceiver (gRPC+HTTP), batchprocessor, otlpexporter, fileexporter (for persistence)
  - No vendor lock-in: use OTLP standard, can swap backends (Jaeger, Prometheus, Loki, etc.)
  Date: 2026-01-23
  Priority: [#A] - foundational change enabling better observability

** [#A] Research and design OpenTelemetry integration architecture
** [#B] Build custom OpenTelemetry Collector binary with ocb
** [#B] Implement Collector lifecycle management in Gestalt
** [#B] Integrate Collector start into Temporal workflow
** [#A] Add OpenTelemetry SDK to Gestalt Go backend
** [#A] Add comprehensive API instrumentation with agent-specific metrics
   Effort: 2-3 days
   Why: Need detailed observability of API performance, agent behavior, and usage patterns for profiling and optimization
   Change:
   - Create internal/otel/middleware.go with HTTP instrumentation middleware:
     ```go
     type APIMetrics struct {
         requestCounter    metric.Int64Counter      // Total requests by endpoint, method, status
         requestDuration   metric.Float64Histogram  // Response time distribution
         requestSize       metric.Int64Histogram    // Request body size
         responseSize      metric.Int64Histogram    // Response body size
         activeRequests    metric.Int64UpDownCounter // Current in-flight requests
         errorCounter      metric.Int64Counter      // Errors by endpoint, error type
     }

     func NewAPIInstrumentationMiddleware(meter metric.Meter) func(http.Handler) http.Handler
     ```
   - Capture semantic HTTP attributes (follow OTel HTTP conventions):
     - http.method (GET, POST, DELETE, etc.)
     - http.route (template: /api/terminals/:id, not actual ID)
     - http.status_code (200, 404, 500, etc.)
     - http.scheme (http/https)
     - http.target (full path with query params, sanitized)
     - user_agent (client type: browser, CLI, etc.)
   - Add agent identification attributes:
     - agent.id (from Authorization header or request context)
     - agent.name (resolved from agent profiles)
     - agent.type (copilot, codex, promptline, human)
     - terminal.id (for terminal-specific endpoints)
   - Parse agent identity from requests:
     - Check Authorization header for token
     - Map token → agent via session lookup
     - For /api/agents/:name/input: extract agent from URL path
     - For /api/terminals/:id: resolve terminal → agent mapping
     - Fallback: agent.type="unknown" if no auth or mapping
   - API categorization (custom attributes):
     - api.category: "terminals", "agents", "workflows", "config", "logs", "skills", "plan", "status"
     - api.operation: "create", "read", "update", "delete", "stream", "query"
   - Metrics to collect:
     - http.server.request.count (counter) - by route, method, status, agent.name, api.category
     - http.server.request.duration (histogram, seconds) - by route, agent.name, api.category
       - Buckets: [0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0] (1ms to 10s)
     - http.server.request.size (histogram, bytes) - request body size
     - http.server.response.size (histogram, bytes) - response body size
     - http.server.active_requests (gauge) - current in-flight requests
     - api.errors.count (counter) - by route, error_type, agent.name
   - Create distributed traces for all API requests:
     - Span: http.server.request (one per HTTP request)
     - Span attributes: all HTTP semantic attributes + agent attributes
     - Link to existing trace context if client sends traceparent header
     - Propagate trace to downstream operations (database, Temporal, etc.)
   - Add span events for key operations:
     - request.body_read, request.validated, response.prepared, response.sent
     - auth.token_validated, agent.resolved, terminal.session_found
   - Instrument critical endpoints explicitly:
     - POST /api/terminals - terminal.create span with agent.id attribute
     - DELETE /api/terminals/:id - terminal.delete span
     - POST /api/agents/:name/input - agent.input span with input size
     - GET /api/terminals/:id/output - terminal.output span with output size
     - WebSocket /ws/terminal/:id - websocket.connect span (separate instrumentation)
   - Add error tracking:
     - Capture error details as span events
     - Record error type, message, stack trace (if available)
     - Increment error counter with error_type label
   - Performance optimization:
     - Use attribute caching: resolve agent.name once, cache for request duration
     - Lazy attribute resolution: don't resolve agent if not needed for metric
     - Sampling: consider tail-based sampling for high-traffic endpoints
   - Dashboard queries enabled by this instrumentation:
     - "Most called APIs" - group by api.route, count requests
     - "Slowest endpoints" - p99 of http.server.request.duration by route
     - "Agent API usage" - group by agent.name, count requests
     - "Error rate by endpoint" - api.errors.count / http.server.request.count
     - "Agent efficiency" - requests per agent, avg duration per agent
     - "API category breakdown" - requests by api.category (pie chart)
     - "Request size distribution" - histogram of request/response sizes
   - Add GET /api/metrics/summary endpoint:
     - Returns JSON summary of API metrics (for UI display)
     - Top 10 endpoints by request count
     - Top 10 slowest endpoints (p99 latency)
     - Top 10 agents by request count
     - Error rate by category
     - Refresh every 60s, cache results
   Tests:
   - Unit tests for middleware: verify metrics recorded, attributes set correctly
   - Integration tests: make requests with/without auth, verify agent resolution
   - Load test: verify overhead <5% with instrumentation enabled
   Done when:
   - All REST endpoints instrumented with OTel middleware
   - Metrics captured: request count, duration, size, errors, active requests
   - Agent identification working: requests tagged with agent.name, agent.id
   - API categorization working: all endpoints tagged with api.category
   - Traces created for all requests, linked to downstream operations
   - Dashboard queries documented and tested
   - /api/metrics/summary returns useful aggregations
** [#A] Migrate internal/logging to OpenTelemetry LogRecords
** [#B] Migrate event.Bus events to OpenTelemetry LogRecords
** [#B] Add distributed tracing for WebSocket connections
** [#B] Integrate OpenTelemetry with Temporal workflows
** [#A] Replace internal/metrics with OpenTelemetry metrics
** [#B] Create frontend OTel query client
** [#B] Update LogsView to use OpenTelemetry logs
** [#C] Create trace viewer component
** [#C] Add OpenTelemetry metrics dashboard
** [#B] Implement parallel run during migration
** [#B] Cut over to OpenTelemetry (remove custom systems)
** [#C] Add remote backend export capability
** [#C] Performance optimization and resource limits
** [#C] Update documentation for OpenTelemetry observability

* DONE [#B] Remove config manifest and fix coder agent extraction
  Goal: Stop relying on config/manifest.json, extract all embedded config files,
  and restore missing agents (including coder) during startup extraction.

** DONE [#B] Update config extraction to walk embedded files without hashes
** DONE [#B] Remove manifest usage in startup and adjust tests

* DONE [#A] Fix agent config validation, CLI config flattening, and log viewer UX
  Goal: Provide actionable validation errors, flatten CLI config keys, log shell execution details, and improve frontend log filtering.

** DONE [#A] Emit detailed validation errors with paths and actual values
** DONE [#A] Flatten agent config keys (no cli_config section) and ensure shell args include all keys
** DONE [#A] Log rendered shell commands + stderr on failure
** DONE [#B] Update log viewer defaults and debug label
** DONE [#B] Expand log detail view to show full event payload

* DONE [#B] CLI-specific agent configuration with TOML format and schema validation
  Goal: Refactor agent configuration system to support per-CLI-type config directives
  with TOML format, JSON Schema validation using invopop/jsonschema, and comprehensive
  codex flag support.

  Context:
  - Current agent config is JSON-based with generic llm_type field
  - Need CLI-specific configuration directives (e.g., codex flags, copilot switches)
  - First major implementation targets codex CLI with full flag support from source
  - Schema validation should catch errors early with clear, actionable messages
  - TOML format is more readable and maintainable than JSON

  Architecture decisions:
  - CLI type remains string field (e.g., "codex", "copilot", "promptline")
  - New optional map field for CLI-specific config: cli_config map[string]interface{}
  - Schemas live in internal/agent/schemas/ with one schema per CLI type
  - invopop/jsonschema generates schemas from Go structs (type-safe, no drift)
  - Validation happens at agent load time with detailed error reporting
  - Shell command construction uses CLI config to append flags

  Backward compatibility:
  - JSON agent files are no longer supported; TOML is required
  - Existing shell field remains but becomes optional if cli_config present

  Notes:
  - Codex source file (codex-config-mod.rs) provided for reference
  - All pub config::Config fields are settable via -c key:value
  - Schema validation points to exact field/line for errors
  - TOML supports comments for better documentation

** [#B] Add invopop/jsonschema dependency and create schema infrastructure
   Why: Need schema validation foundation before implementing CLI configs

   Change:
   1. Add invopop/jsonschema to go.mod:
      - Run: go get github.com/invopop/jsonschema
   2. Create internal/agent/schemas/ directory structure:
      - schemas/
        - codex.go (CodexConfig struct + schema generation)
        - copilot.go (CopilotConfig struct + schema generation)
        - common.go (shared validation helpers)
        - generator.go (schema generation utility)
   3. Create internal/agent/validation.go:
      - ValidateAgentConfig(cliType string, config map[string]interface{}) error
      - Detailed error formatting with field path and expected type
      - Schema loading/caching mechanism

   Tests:
   - Unit tests for schema generation (schemas/generator_test.go)
   - Validation error formatting tests (validation_test.go)
   - Edge cases: unknown fields, type mismatches, required fields

   Done when:
   - go get succeeds without conflicts
   - internal/agent/schemas/ directory exists with generator
   - Validation function returns clear errors for bad configs
   - All tests pass

** [#B] Implement codex configuration schema from codex-config-mod.rs
   Why: Codex is the primary target CLI with extensive configuration options

   Change:
   1. Extract all pub fields from codex-config-mod.rs Config struct (lines 96-374)
   2. Create internal/agent/schemas/codex.go with CodexConfig struct:
      - Map Rust types to Go equivalents (bool, string, i64->int64, Option<T>->*T)
      - Add json schema annotations for validation
      - Common flags as direct fields (model, approval_policy, sandbox_mode, etc.)
      - Nested structs for complex config (shell_environment_policy, tui, features, etc.)
   3. Document each field with comments from Rust source
   4. Implement MarshalJSONSchema() for custom schema generation
   5. Create codex_test.go with validation tests:
      - Valid config examples
      - Invalid type scenarios
      - Unknown field warnings
      - Required field enforcement

   Reference fields (partial list from codex-config-mod.rs):
   - model (string)
   - review_model (string)
   - model_provider (string)
   - model_context_window (int64)
   - approval_policy (string: "never", "on-failure", "untrusted", "on-request")
   - sandbox_mode (string: "read-only", "workspace-write", "danger-full-access")
   - shell_environment_policy (nested)
   - hide_agent_reasoning (bool)
   - show_raw_agent_reasoning (bool)
   - tui settings (nested: animations, notifications, scroll_*, alternate_screen)
   - features (nested: enable/disable experimental features)
   - profile (string)
   - check_for_update_on_startup (bool)
   - analytics (nested: enabled)
   - And 50+ more fields...

   Tests:
   - Schema generation produces valid JSON Schema
   - All Rust config fields represented in Go struct
   - Type validation catches common errors
   - Nested config validates recursively

   Done when:
   - CodexConfig struct has all fields from Rust Config
   - Schema generation works and validates correctly
   - Tests cover major config scenarios
   - Documentation matches Rust comments

** [#C] Implement copilot configuration schema
   Why: Second major CLI requiring configuration support

   Change:
   1. Research copilot CLI flags:
      - Run copilot --help to extract available flags
      - Document in schemas/copilot.go comments
   2. Create internal/agent/schemas/copilot.go with CopilotConfig:
      - Common flags: --model, --allow-all-tools, --disable-builtin-mcps
      - MCP configuration options
      - Output verbosity controls
      - Auth/workspace settings
   3. Implement schema generation and validation
   4. Create copilot_test.go with validation tests

   Tests:
   - Valid copilot configs validate successfully
   - Unknown flags produce warnings
   - Required fields enforced

   Done when:
   - CopilotConfig struct defined with copilot flags
   - Schema validates copilot-specific configs
   - Tests pass

** [#B] Extend Agent struct to support cli_config and TOML format
   Why: Core data model needs to accommodate new configuration approach

   Change:
   1. Update internal/agent/agent.go Agent struct:
      - Rename llm_type to cli_type (clearer semantics)
      - Add CLIConfig map[string]interface{} `toml:"cli_config" json:"cli_config,omitempty"`
      - Make Shell optional (can be generated from cli_config)
      - Add TOML struct tags to all fields
      - Add ConfigHash string field (FNV-1a 64-bit hex, same as config/extractor.go)
   2. Update Validate() method:
      - Call ValidateAgentConfig(a.CLIType, a.CLIConfig) if cli_config present
      - Generate shell command from cli_config if Shell empty
      - Warn if both shell and cli_config present (cli_config takes precedence)
   3. Implement BuildShellCommand(cliType string, config map[string]interface{}) string:
      - Switch on cliType
      - For "codex": construct command with -c key:value flags
      - For "copilot": construct command with --flag syntax
      - Handle nested configs (flatten to CLI args)
   4. Implement ComputeConfigHash(agent *Agent) string:
      - Serialize agent config to canonical form (sorted JSON)
      - Hash using fnv.New64a() (same as config extractor)
      - Return as "%016x" hex string
      - Used for change detection
   5. Update agent_test.go:
      - Test TOML parsing
      - Test shell command generation from cli_config
      - Test config hash computation and stability

   Tests:
   - TOML parsing works with cli_config
   - Shell generation creates valid commands
   - Validation catches schema violations
   - Config hash is consistent for same config

   Done when:
   - Agent struct supports cli_config field with hash
   - Shell command generation works for codex and copilot
   - Config hash function works reliably
   - All tests pass including backward compat
   - Deprecation warning logged for JSON files

** [#B] Update agent loader to support TOML files with proper validation
   Why: Need to load TOML agent configs and validate against schemas

   Change:
   1. Add github.com/BurntSushi/toml dependency
   2. Update internal/agent/loader.go LoadFromDir():
      - Support *.toml file extension only
      - Fail fast on non-TOML agent files
      - Call ValidateAgentConfig after parsing
      - Compute and store ConfigHash in each loaded agent
      - Include filename and line number in validation errors
   3. Create formatValidationError() helper:
      - Extract field path from schema error
      - Format as: "agent 'Coder' in codex.toml: cli_config.model: expected string, got number"
      - Include expected type and actual value
   4. Add LoadAgentByID(agentID string, agentsDir string) (*Agent, error):
      - Load single agent config by ID
      - Used for reload-before-start check
      - Return error if not found or invalid
   5. Update loader_test.go:
      - Test TOML loading
      - Test validation error messages
      - Test mixed extension rejection
      - Test schema violation scenarios
      - Test single agent reload

   Tests:
   - TOML files load successfully
   - Non-TOML files are rejected
   - Validation errors are actionable
   - Invalid TOML syntax produces clear error
   - Schema violations point to exact field
   - Single agent reload works

   Done when:
   - LoadFromDir handles *.toml files
   - Validation errors are clear and helpful
   - Only TOML agent configs are accepted
   - LoadAgentByID works for config refresh
   - All loader tests pass

** [#C] Implement codex shell command generation from cli_config
   Why: Transform cli_config into actual codex command with flags

   Change:
   1. Create internal/agent/shellgen/ package:
      - shellgen/codex.go
      - shellgen/copilot.go
      - shellgen/common.go
   2. Implement BuildCodexCommand(config CodexConfig) []string:
      - Start with base: ["codex"]
      - Iterate config fields, append as -c key:value
      - Handle nested configs (flatten with dot notation)
      - Handle boolean flags (presence vs -c flag:true)
      - Handle arrays (repeat -c flag for each item)
      - Examples:
        model: "o3" -> -c model:o3
        approval_policy: "never" -> -c approval_policy:never
        tui.animations: false -> -c tui.animations:false
   3. Create shellgen/codex_test.go:
      - Test simple config -> command
      - Test nested config flattening
      - Test boolean and array handling
      - Test shell escaping for special chars

   Tests:
   - Simple configs produce correct commands
   - Nested configs flatten properly
   - Special characters are escaped
   - Edge cases (empty config, nil values) handled

   Done when:
   - BuildCodexCommand generates valid codex commands
   - All codex flags from config work correctly
   - Tests cover common and edge cases
   - Shell escaping is safe

** [#C] Implement copilot shell command generation from cli_config
   Why: Transform cli_config into copilot command with flags

   Change:
   1. Implement BuildCopilotCommand(config CopilotConfig) []string:
      - Start with base: ["copilot"]
      - Append flags using --flag or --flag=value syntax
      - Handle boolean flags (--allow-all-tools vs --no-allow-all-tools)
      - Handle repeated flags for arrays
   2. Create shellgen/copilot_test.go:
      - Test simple config -> command
      - Test boolean flag syntax
      - Test flag precedence

   Tests:
   - Copilot configs produce correct commands
   - Boolean flags use correct syntax
   - Tests cover common scenarios

   Done when:
   - BuildCopilotCommand generates valid copilot commands
   - Boolean flags handled correctly
   - Tests pass

** [#B] Add config refresh before terminal session start
   Why: Ensure NEW agent sessions always launch with latest config from disk

   IMPORTANT: Config refresh only affects NEW session creation. Running sessions
   are NEVER touched or modified. Each session is immutable once started, using
   the config snapshot from its creation time. Config changes only apply to the
   NEXT session created for that agent.

   Change:
   1. Create internal/agent/cache.go:
      - AgentCache struct with map[agentID]*Agent and sync.RWMutex
      - LoadOrReload(agentID string, agentsDir string) (*Agent, bool, error)
        - Returns (agent, wasReloaded, error)
        - Check if agent in cache and hash matches disk
        - If not cached or hash differs, reload from disk
        - Update cache on successful load
      - Clear() method for testing
      - Document: "Cache only affects NEW session creation, not running sessions"
   2. Update internal/terminal/manager.go:
      - Add agentCache *agent.AgentCache field
      - Initialize in NewManager()
      - Pass agentsDir path to manager
      - Document: Manager never modifies running sessions
   3. Update internal/api/rest.go handleCreateTerminal():
      - Before creating NEW session, call agentCache.LoadOrReload(agentID)
      - Log if config was reloaded: "agent '%s' config reloaded for new session (hash changed)"
      - Pass fresh agent to NewSession()
      - Clarify: Existing sessions continue with their original config
   4. Create cache_test.go:
      - Test cache hit (same hash)
      - Test cache miss (new agent)
      - Test cache reload (hash changed)
      - Test concurrent access safety
      - Test that running sessions are unaffected by reload

   Tests:
   - Cache correctly detects config changes via hash
   - Concurrent reloads are safe
   - Cache miss triggers reload
   - Hash match uses cached agent
   - Running sessions unaffected by config changes

   Done when:
   - Agent configs reload before each NEW session start
   - Cache prevents redundant disk reads
   - Logging shows when reload happens for new sessions
   - Running sessions explicitly left untouched
   - All cache tests pass

** [#B] Update terminal session creation to use generated shell commands
   Why: Connect new cli_config system to actual terminal launching

   IMPORTANT: Each session is a snapshot of the agent config at creation time.
   Once a session is created, it NEVER changes. The session runs with the exact
   config (shell command, cli_config, hash) that was loaded at creation time.
   Config changes only affect NEW sessions, never existing ones.

   Change:
   1. Update internal/terminal/session.go NewSession():
      - Accept fresh agent from cache (may have different config than previous sessions)
      - If agent has cli_config, call BuildShellCommand() to get command args
      - Otherwise use agent.Shell as before (backward compat)
      - Store agent.ConfigHash in Session struct (immutable snapshot)
      - Log generated command at debug level
      - Document: Session config is immutable after creation
   2. Update Session struct:
      - Add ConfigHash string field
      - Store full agent config hash for reference
      - Document: ConfigHash is snapshot from creation time, never updated
   3. Update internal/api/rest.go handleCreateTerminal():
      - Pass reloaded agent (from cache) to NEW session creation
      - Include generated command in response (optional field)
      - Document: Only new sessions use reloaded config
   4. Add terminal_test.go test for cli_config agents:
      - Mock agent with cli_config
      - Verify correct command is spawned
      - Test config hash stored in session
      - Test that session config doesn't change after creation

   Tests:
   - Sessions created with cli_config use generated command
   - Sessions with old Shell field still work
   - Command logging helps debugging
   - Config hash stored in session immutably
   - Session config never changes after creation

   Done when:
   - Terminal sessions use shell commands from fresh config at creation
   - Backward compat with Shell field maintained
   - Config hash stored in session as immutable snapshot
   - Session immutability documented and tested
   - Tests verify correct behavior

** [#C] Document new configuration system in docs/ and AGENTS.md
   Why: Users and LLM assistants need clear documentation

   Change:
   1. Create docs/agent-configuration.md:
      - Overview of TOML format
      - CLI-specific config sections
      - Schema validation behavior
      - Shell command generation process
      - Full codex config field reference
      - Copilot config examples
   2. Update AGENTS.md with new section:
      - "Agent configuration format (TOML)" section
      - CLI-specific configs vs generic shell field
      - Schema validation and error messages
      - Example TOML files for common use cases
   3. Add inline examples in config/agents/:
      - codex-full-example.toml (commented with all options)
      - copilot-example.toml
      - simple-shell-example.toml (backward compat)

   Tests: N/A (documentation only)

   Done when:
   - docs/agent-configuration.md complete
   - AGENTS.md updated with TOML format
   - Example TOML files provided
   - All config fields documented

** [#C] Add integration tests for full config validation workflow
   Why: Ensure end-to-end config system works correctly

   Change:
   1. Create internal/agent/integration_test.go:
      - Test loading TOML agents from disk
   ...
   2. Create test fixtures in testdata/:
      - valid-codex.toml
      - invalid-codex-type.toml (wrong type)
      - invalid-codex-unknown.toml (unknown field)
      - valid-copilot.toml
   3. Test error messages:
      - Verify actionable error text
      - Verify field paths are correct
      - Verify line numbers when possible

   Tests:
   - All integration tests pass
   - Error messages are helpful
   - Mixed extension rejection behaves correctly

   Done when:
   - Integration tests cover full workflow
   - Test fixtures represent real scenarios
   - Error messages verified to be clear
   - All tests pass consistently

** [#C] Add CLI validation command for config files
   Why: Let users validate configs before runtime

   Change:
   1. Create cmd/gestalt/validate_config.go:
      - Command: gestalt config validate
      - Flags: --agents-dir (default .gestalt/config/agents)
      - Load all agent configs
      - Run validation on each
      - Print summary: X valid, Y invalid with details
   2. Exit code:
      - 0 if all valid
      - 1 if any invalid
   3. Output format:
      - Green checkmark for valid agents
      - Red X with error details for invalid
      - Summary line at end

   Tests:
   - Valid directory returns 0
   - Invalid config returns 1 with details
   - Empty directory handled gracefully

   Done when:
   - gestalt config validate works
   - Clear success/failure output
   - Useful for CI pipelines
   - Tests pass

** [#A] Update default agent configs to use TOML with cli_config
   Why: Shipping default configs should use new format

   Change:
   1. Convert config/agents/*.json to *.toml:
      - codex.toml with full cli_config
      - copilot.toml with full cli_config
      - fixer.toml with cli_config
      - promptline.toml (if applicable)
   2. Add extensive comments to each:
      - Explain each cli_config field
      - Link to docs for more details
      - Show commented-out optional fields
   3. Update config/manifest.json:
      - Include *.toml files
      - Update hashes
   4. Update config extraction in cmd/gestalt/main.go:
      - Ensure TOML files extract correctly
   5. Remove JSON versions in config/ to avoid confusion

   Tests:
   - Embedded TOML configs load successfully
   - Default agents start with correct commands
   - Config extraction handles TOML

   Done when:
   - Default configs are TOML with cli_config
   - Embedded agents work out-of-box
   - Extraction preserves TOML format
   - All agents start successfully

** [#B] Store agent config and hash in Temporal workflow memo

** [#C] Tighten memo JSON error and warn on legacy memo blobs
   Why: Make memo format changes explicit and surface legacy JSON memo blobs during validation.

   Change:
   1. Update internal/temporal/memo.go:
      - Detect JSON-looking memo payloads and return a clear error message
      - TOML-only parsing (no JSON fallback)
   2. Update cmd/gestalt/validate_agents.go:
      - Scan for legacy memo blobs in the local Temporal dev dir (if present)
      - Emit a migration warning when JSON agent_config memos are detected
   3. Tests:
      - memo_test.go asserts JSON rejection message
      - validate_agents_test.go covers warning emission when legacy memo data exists

   Done when:
   - JSON-looking memos produce explicit errors
   - Validation prints a migration warning when legacy memo blobs are found
   - Tests pass
   Why: Make agent config traceable in workflow history for debugging and auditing

   IMPORTANT: Workflow memo captures the EXACT agent config used at session creation.
   This is an immutable snapshot. If the agent config changes on disk later, running
   workflows are unaffected. The memo preserves what config was actually used to
   launch each specific session.

   Change:
   1. Update internal/temporal/workflows/session_workflow.go:
      - Extend SessionWorkflowRequest struct:
        - Add AgentConfig string (serialized TOML or JSON)
        - Add ConfigHash string (FNV-1a 64-bit hex)
      - Document that config is stored as workflow memo for auditability
      - Document that memo is immutable snapshot from session creation time
   2. Update internal/terminal/session.go startWorkflow():
      - Serialize agent config to TOML string (preferred) or JSON fallback
      - Pass agent.ConfigHash to SessionWorkflowRequest
      - Use Temporal Memo to store config metadata:
        - client.StartWorkflowOptions.Memo = map[string]interface{}{
            "agent_config": serializedConfig,
            "config_hash": agent.ConfigHash,
            "cli_type": agent.CLIType,
            "created_at": session.CreatedAt,
          }
      - Memo is immutable workflow metadata, queryable via Temporal API
      - Document: Memo preserves config snapshot from session creation
   3. Create internal/temporal/memo.go helper:
      - SerializeAgentConfig(agent *Agent) (string, error)
        - Try TOML serialization first
        - Fall back to JSON if TOML fails
        - Keep under 2KB (Temporal memo size limit)
      - DeserializeAgentConfig(data string) (*Agent, error)
        - Try TOML parse first, then JSON
        - Return error if neither works
      - Document: Config in memo is creation-time snapshot
   4. Update session_workflow_test.go:
      - Test workflow stores config in memo
      - Test memo retrieval via GetWorkflowInfo
      - Test config serialization/deserialization round-trip
      - Test that memo is immutable (doesn't change with config file updates)

   Reference: Temporal best practices
   - Memo: Small, immutable metadata attached to workflow execution
   - Used for: audit trails, debugging, filtering workflows
   - Queryable via: WorkflowExecution API, tctl, Temporal Web UI
   - Size limit: ~2KB per memo entry
   - Alternative: SearchAttributes for indexing (not needed here)
   - Memo is snapshot: doesn't change even if source data changes

   Tests:
   - Workflow memo contains agent config and hash
   - Config can be retrieved from completed workflows
   - Serialization round-trips correctly
   - Large configs are truncated gracefully
   - Memo immutability verified

   Done when:
   - Workflows store agent config snapshot in memo
   - Config hash stored for quick comparison
   - Serialization handles TOML and JSON
   - Tests verify memo storage and retrieval
   - Temporal UI shows config in workflow details
   - Documentation clarifies memo is creation-time snapshot
* DONE [#A] SCIP code intelligence integration for LLM context enhancement
  Goal: Integrate Sourcegraph's SCIP (SCIP Code Intelligence Protocol) indexing into Gestalt to provide rich code context to LLM agents through symbol-aware code search and retrieval.

  ## Implementation Summary (for coder)

  This L1 adds **code intelligence** to Gestalt agents using SCIP indexes. Agents can query code symbols (functions, types, variables) with semantic understanding rather than plain text search. This dramatically improves LLM context quality by providing relevant code snippets with type information, documentation, and relationships.

  **What SCIP Provides:**
  - Symbol definitions with signatures and documentation
  - Symbol occurrences (definitions, references, implementations)
  - Symbol relationships (calls, inheritance, type definitions)
  - Language-agnostic format (Go, TypeScript, Python, Java, etc)
  - Efficient protobuf-based storage
  - SQLite export for queryability

  **Architecture Decision: Built-in Module (not separate MCP server)**
  - Reasoning: Simpler deployment, tighter integration, less overhead
  - SCIP queries exposed as REST API endpoints (same pattern as existing APIs)
  - Agents use SCIP via HTTP (no new protocol)
  - Later: can add MCP adapter if external tools need it

  **Key Components:**
  1. **Indexer Management**: Auto-download and run language-specific indexers
  2. **SQLite Conversion**: Convert SCIP protobuf → queryable SQLite DB
  3. **Query API**: Go package `internal/scip` with symbol search/retrieval
  4. **REST Endpoints**: `scip-api/symbols`, `scip-api/references`, etc
  5. **Agent Integration**: Prompts document SCIP APIs for agent use

  **Implementation Steps Overview:**
  1. Add `internal/scip` package with SCIP → SQLite conversion
  2. Implement indexer downloader (embedded URLs for scip-go, scip-typescript, etc)
  3. Build query API: FindSymbols, GetReferences, GetDefinition, GetDocumentation
  4. Expose REST endpoints for agent access
  5. Add indexing commands: `gestalt index` (or auto-index on startup)
  6. Document SCIP API usage in agent prompts (skill or prompt template)
  7. Test with real codebases (Gestalt itself, example repos)

  **Cody's Pattern (what we're replicating):**
  1. Agent asks question about code
  2. Extract key symbols/types from question
  3. Query SCIP for definitions and related symbols
  4. Retrieve source snippets (20-50 lines around each symbol)
  5. Rank by relevance (symbol name match, relationship strength)
  6. Include top N results in LLM context
  7. LLM answers with precise knowledge of codebase

  **No Online Research Needed:**
  All SCIP concepts, indexer URLs, SQLite schema, and API patterns documented below in L2 tasks. Coder can implement from these instructions.

  Notes:
  - SCIP is production-ready (used by Sourcegraph Cody, mature ecosystem)
  - Indexes are language-agnostic (single format for all languages)
  - SQLite export is experimental but stable enough for our use
  - Gestalt repo already has index.scip (108 documents, 31k occurrences)
  - Indexing is **on-demand** (not continuous), triggered by user or cron
  - Agents access SCIP via REST API (same auth as other endpoints)
  - Future: could add MCP server for external tool integration
  - Repository language detection: use go.mod, package.json, etc for initial pass
  - Multi-language repos: run multiple indexers, merge indexes
  - DEFER: Downloader URLs and asset handling live in internal/scip/indexers.go (indexerURL/downloadBinary); revisit when embedding assets.
  Date: 2026-01-12
  Priority: High - directly improves agent code understanding

** [#B] Research SCIP format and understand SQLite schema
   Why: Must understand SCIP data model before building query layer
   Change:
   - Read SCIP protobuf schema: https://github.com/sourcegraph/scip/blob/main/scip.proto
   - Understand key concepts:
     - **Document**: A source file with language and relative path
     - **Occurrence**: A symbol reference at specific location (range)
       - SymbolRoles: Definition=1, Import=2, WriteAccess=4, ReadAccess=8, etc
     - **Symbol**: Unique identifier string (e.g., `scip-go gomod gestalt pkg/Manager#Start()`)
       - Format: `scheme language package [descriptor...] symbol`
     - **SymbolInformation**: Definition with signature, documentation, kind
       - Kind: Package, Class, Method, Function, Variable, etc
     - **Relationship**: Symbol relationships (implementation, type_definition, reference)
   - Install SCIP CLI (if not present):
     ```bash
     go install github.com/sourcegraph/scip/cmd/scip@latest
     ```
   - Experiment with existing index.scip:
     ```bash
     scip stats index.scip          # Statistics
     scip print index.scip | less   # Human-readable dump
     scip lint index.scip           # Validation
     ```
   - Convert to SQLite and inspect schema:
     ```bash
     scip expt-convert --output index.db index.scip
     # Inspect schema (need SQLite tool or Go code)
     ```
   - Document SQLite schema (reverse-engineer from conversion):
     - `metadata` table: version, tool_info, project_root
     - `documents` table: id, path, language
     - `symbols` table: id, symbol_name, documentation, signature, kind
     - `occurrences` table: document_id, symbol_id, range_blob, role
     - `relationships` table: from_symbol, to_symbol, relationship_type
   - Note: Occurrences stored as protobuf blob for space efficiency
   - Document how to decode occurrence blobs (need SCIP protobuf library)
   - Identify what queries we need:
     1. Find symbols by name (fuzzy match)
     2. Get symbol definition (with documentation)
     3. Find all references to symbol
     4. Find symbols in file/package
     5. Get type of symbol (for type-aware queries)
     6. Find implementations of interface/trait
     7. Find callers of function
   Tests: None (research and documentation)
   Done when: SCIP format understood, SQLite schema documented, query needs identified

** [#B] Create internal/scip package with SQLite query layer
   Why: Need Go API to query SCIP indexes efficiently
   Change:
   - Create internal/scip/index.go:
     ...
     type Occurrence struct {
         ...
         Role     string `json:"role"` // "definition", "reference", "implementation"
     }

     // FindSymbols searches for symbols by name (fuzzy match)
     func (idx *Index) FindSymbols(query string, limit int) ([]Symbol, error) {
         // Implementation: query symbols table with LIKE or FTS
         // Return symbols matching query, ranked by relevance
     }

     // GetDefinition retrieves symbol definition with documentation
     func (idx *Index) GetDefinition(symbolID string) (*Symbol, error) {
         // Implementation: join symbols + documents + occurrences
         // Find definition occurrence, extract location and metadata
     }

     // GetReferences finds all references to a symbol
     func (idx *Index) GetReferences(symbolID string) ([]Occurrence, error) {
         // Implementation: query occurrences where symbol = symbolID
         // Decode range blobs, extract file paths and positions
     }

     // GetSymbolsInFile lists all symbols defined in a file
     func (idx *Index) GetSymbolsInFile(filePath string) ([]Symbol, error) {
         // Implementation: join documents + occurrences (definition role only)
     }

     // GetTypeInfo retrieves type information for a symbol
     func (idx *Index) GetTypeInfo(symbolID string) (string, error) {
         // Implementation: extract type from signature or relationships
     }
     ```
   - Add dependency: `github.com/mattn/go-sqlite3` (pure Go SQLite)
   - Implement helper to decode occurrence range blobs:
     - Ranges are int32 arrays: [startLine, startCol, endCol] or [startLine, startCol, endLine, endCol]
     - May need to import SCIP protobuf definitions
   - Add internal/scip/convert.go to wrap `scip expt-convert`:
     ```go
     // ConvertToSQLite runs scip CLI to convert .scip to .db
     func ConvertToSQLite(scipPath, dbPath string) error {
         cmd := exec.Command("scip", "expt-convert", "--output", dbPath, scipPath)
         return cmd.Run()
     }
     ```
   - Error handling: return typed errors (SymbolNotFound, IndexCorrupted, etc)
   - Add caching: LRU cache for frequently accessed symbols
   - Tests:
     - Unit tests with test fixture index (small Go project)
     - Test FindSymbols with exact and fuzzy matches
     - Test GetDefinition returns correct file/line
     - Test GetReferences finds all occurrences
   Done when: internal/scip package queries SQLite successfully with tests

** [#B] Implement indexer downloader and language detection
   Why: Need to automatically download and run appropriate SCIP indexers
   Change:
   - Create internal/scip/indexers.go:
     ```go
     package scip
     ...
     type Indexer struct {
         ...
         Binary   string // Binary name (e.g., "scip-go")
     }
     ...
     var builtInIndexers = []Indexer{
         {
             ...
             Name:     "scip-go",
             Version:  "v0.1.26",
             URL:      "https://github.com/sourcegraph/scip-go/releases/download/v0.1.26/scip-go-linux-amd64",
             Binary:   "scip-go",
         },
         {
             Language: "typescript",
             ...
             Version:  "v0.3.13",
             URL:      "https://github.com/sourcegraph/scip-typescript/releases/download/v0.3.13/scip-typescript-linux-x64",
         ...
         },
         {
             ...
             Name:     "scip-python",
             Version:  "v0.3.9",
             URL:      "https://github.com/sourcegraph/scip-python/releases/download/v0.3.9/scip-python-linux-x64",
             Binary:   "scip-python",
         },
         // Add more as needed
     }

     // DownloadIndexer downloads indexer binary to ~/.gestalt/indexers/
     func DownloadIndexer(lang string) error {
         indexer := findIndexer(lang)
         destPath := filepath.Join(getIndexerDir(), indexer.Binary)
         // Download from URL, make executable
         return downloadBinary(indexer.URL, destPath)
     }

     // DetectLanguages scans directory for language markers
     func DetectLanguages(dir string) ([]string, error) {
         ...
         // Python: setup.py, requirements.txt, or *.py files
     ...
     // RunIndexer executes indexer on directory, outputs index.scip
     func RunIndexer(lang, dir, output string) error {
         indexerPath := filepath.Join(getIndexerDir(), "scip-"+lang)

         // Ensure indexer is downloaded
         ...
         // Run indexer: scip-go index (for Go), scip-typescript index, etc
         cmd := exec.Command(indexerPath, "index")
         cmd.Dir = dir
         // Indexers typically output to index.scip in current dir
         // May need to rename/move to output path
         return cmd.Run()
     }
     ```
   - Add platform detection (Linux/macOS/Windows) to select correct binary URL
   - Embed indexer URLs in code (no external config needed)
   - Support multi-platform binaries:
     - Linux: -linux-amd64
     - macOS: -darwin-arm64, -darwin-amd64
     - Windows: -windows-amd64.exe
   - Add `gestalt index [path]` command:
     - Auto-detect languages in path
     - Download missing indexers
     - Run indexers, generate index.scip
     - Convert to SQLite: index.db
   - Error handling: missing indexer, download failure, indexing failure
   - Progress reporting: log indexing steps
   Tests:
     - Test language detection on test repos
     - Test indexer download (mock HTTP)
     - Test indexing (with real indexer if available)
   Done when: Indexers auto-download and run on detected languages

** [#B] Add REST API endpoints for SCIP queries
   Why: Agents need HTTP access to SCIP data
   Change:
   - Create internalscip-api.go:
     ```go
     package api
     ...
     func NewSCIPHandler(indexPath string) (*SCIPHandler, error) {
         idx, err := scip.OpenIndex(indexPath)
         if err != nil {
             return nil, err
         }
         return &SCIPHandler{index: idx}, nil
     }

     // GET scip-api/symbols?q=FindUser&limit=10
     func (h *SCIPHandler) FindSymbols(w http.ResponseWriter, r *http.Request) {
         query := r.URL.Query().Get("q")
         limit := getIntParam(r, "limit", 20)

         symbols, err := h.index.FindSymbols(query, limit)
         if err != nil {
     ...
     // GET scip-api/symbols/:id
     func (h *SCIPHandler) GetSymbol(w http.ResponseWriter, r *http.Request) {
         symbolID := getPathParam(r, "id")

         symbol, err := h.index.GetDefinition(symbolID)
         if err != nil {
     ...
     // GET scip-api/symbols/:id/references
     func (h *SCIPHandler) GetReferences(w http.ResponseWriter, r *http.Request) {
         symbolID := getPathParam(r, "id")

         refs, err := h.index.GetReferences(symbolID)
         if err != nil {
             writeError(w, http.StatusInternalServerError, err)
             return
         }
     ...
     // GET scip-api/files/:path
     func (h *SCIPHandler) GetFileSymbols(w http.ResponseWriter, r *http.Request) {
         filePath := getPathParam(r, "path")

         symbols, err := h.index.GetSymbolsInFile(filePath)
         if err != nil {
             writeError(w, http.StatusInternalServerError, err)
             return
         }
     ...
     // POST scip-api/index
     // Trigger re-indexing of repository
     func (h *SCIPHandler) ReIndex(w http.ResponseWriter, r *http.Request) {
         // Parse request body: { "path": "/repo/path" }
         var req struct {
         ...
         go func() {
             langs, _ := scip.DetectLanguages(req.Path)
             for _, lang := range langs {
                 scip.RunIndexer(lang, req.Path, "index.scip")
             }
             scip.ConvertToSQLite("index.scip", "index.db")
             // Reload index
             h.index, _ = scip.OpenIndex("index.db")
         }()
   ...
   - Register routes in internal/api/routes.go:
     ```go
     scipHandler, err := NewSCIPHandler(config.SCIPIndexPath)
     if err == nil {
         router.HandleFunc("scip-api/symbols", scipHandler.FindSymbols)
         router.HandleFunc("scip-api/symbols/{id}", scipHandler.GetSymbol)
         router.HandleFunc("scip-api/symbols/{id}/references", scipHandler.GetReferences)
         router.HandleFunc("scip-api/files/{path}", scipHandler.GetFileSymbols)
         router.HandleFunc("scip-api/index", scipHandler.ReIndex).Methods("POST")
     }
     ```
   - Add config: GESTALT_SCIP_INDEX_PATH env var (default: `.gestalt/index.db`)
   - Authentication: use existing auth middleware
   - Rate limiting: agents may query frequently, add rate limits
   - Caching: cache common queries (e.g., frequently accessed symbols)
   - Documentation: OpenAPI/Swagger spec for endpoints
   Tests:
     - Integration tests with test index
     - Test all endpoints return correct data
     - Test error handling (missing symbol, corrupt index)
   Done when: REST API exposes SCIP queries to agents

** [#B] Create agent prompt/skill documenting SCIP API usage
   Why: Agents need to know how to query SCIP effectively
   Change:
   - Create .gestalt/config/skills/code-intelligence/SKILL.md:
     ```markdown
     ---
     ...
     Use SCIP (Code Intelligence Protocol) to search and analyze code symbols.

     ## Available APIs

     ### Find Symbols by Name
     ```
     GET scip-api/symbols?q=<query>&limit=<n>
     ```
     Returns symbols matching query (fuzzy match).
     Example: `q=FindUser` finds `FindUser`, `FindUserByID`, etc.

     ### Get Symbol Definition
     ```
     GET scip-api/symbols/<symbol-id>
     ```
     Returns definition with documentation, signature, file location.

     ### Find References
     ```
     GET scip-api/symbols/<symbol-id>/references
     ```
     Returns all places where symbol is referenced.

     ### List Symbols in File
     ```
     GET scip-api/files/<file-path>
     ```
     Returns all symbols defined in file.

     ## Usage Patterns

     **Understanding a function:**
     1. Search for function: `scip-api/symbols?q=ProcessOrder`
     2. Get definition: `scip-api/symbols/<id>` (includes documentation)
     3. Find callers: `scip-api/symbols/<id>/references`
     4. Read source at definition location

     **Finding related code:**
     1. Search for type: `scip-api/symbols?q=User`
     2. Get type definition and fields
     3. Find methods on type (search `User.`)
     4. Find usage patterns via references

     **Exploring a module:**
     1. List symbols in file: `scip-api/files/internal/user/user.go`
     2. For each symbol, get references to see usage
     3. Build call graph mentally

     ## Tips

     - Use specific queries (function names, type names)
     - Start with definition, then explore references
     - Combine with file content API to see implementation
     - Symbol IDs are stable across re-indexing
     - Fuzzy matching helps with typos/partial names
     ```
   - Alternatively: Add to default agent prompt template:
     ```markdown
     ## Code Intelligence (SCIP)

     You have access to code intelligence APIs via SCIP indexes.
     These APIs provide symbol-level understanding of the codebase.

     Available endpoints:
     - GET scip-api/symbols?q=<query> - Search symbols
     - GET scip-api/symbols/<id> - Get definition
     - GET scip-api/symbols/<id>/references - Find usages
     - GET scip-api/files/<path> - List symbols in file

     Use these to understand code structure, find relevant code, trace
     dependencies, and answer questions about implementation details.
     ```
   - Add examples to AGENTS.md for LLMs:
     ```markdown
     ## SCIP Code Intelligence
     - Agents can query SCIP indexes for symbol-aware code search
     - REST API: `scip-api/symbols`, `scip-api/symbols/:id/references`
     - Symbol types: function, type, variable, interface, etc
     - Includes documentation, signatures, relationships
     - Use for: finding definitions, tracing callers, understanding types
     - Example: "Find all callers of ProcessOrder function"
       1. Search: GET scip-api/symbols?q=ProcessOrder
       2. Get ID from results
       3. Query: GET scip-api/symbols/<id>/references
       4. Read source at each reference location
     ```
   Tests: Manual test with agent prompts
   Done when: Agents understand how to use SCIP API

** [#C] Add indexing command to gestalt CLI
   Why: Users need simple way to index their codebases
   Change:
   - Add cmd/gestalt/index.go:
     ```go
     package main
     ...
     func indexCommand() {
         ...
         path := indexFlags.String("path", ".", "Path to index")
         ...
         if !*force && fileExists(*output) {
             fmt.Println("Index exists. Use --force to re-index.")
             return
         }

         // Detect languages
         langs, err := scip.DetectLanguages(*path)
         ...
         // Download indexers if needed
         for _, lang := range langs {
             fmt.Printf("Checking indexer for %s...\n", lang)
             if err := scip.EnsureIndexer(lang); err != nil {
                 fmt.Printf("Warning: Failed to get indexer for %s: %v\n", lang, err)
             }
         }
         ...
         scipIndexes := []string{}
         for _, lang := range langs {
             fmt.Printf("Indexing %s code...\n", lang)
             scipOut := fmt.Sprintf("index-%s.scip", lang)
             if err := scip.RunIndexer(lang, *path, scipOut); err != nil {
                 fmt.Printf("Warning: Indexing %s failed: %v\n", lang, err)
                 continue
             }
             scipIndexes = append(scipIndexes, scipOut)
         }

         // Merge indexes if multiple languages
         var finalScip string
         if len(scipIndexes) == 1 {
             finalScip = scipIndexes[0]
         } else {
             finalScip = "index.scip"
             if err := scip.MergeIndexes(scipIndexes, finalScip); err != nil {
                 fmt.Printf("Error merging indexes: %v\n", err)
                 return
             }
         }

         // Convert to SQLite
         fmt.Printf("Converting to SQLite...\n")
         if err := scip.ConvertToSQLite(finalScip, *output); err != nil {
             fmt.Printf("Error converting to SQLite: %v\n", err)
             return
         }

         // Print stats
         idx, _ := scip.OpenIndex(*output)
         defer idx.Close()
   ...
   - Add to main.go:
     ...
     ```
   - Add --help for index command:
     ```
     gestalt index [options]
       --path      Path to repository (default: current directory)
       --output    Output SQLite path (default: index.db)
   ...
   - Document in README.md:
     ...
     ```
   - Consider auto-indexing on startup (optional):
     - If no index.db exists, offer to index
     - Or add config: GESTALT_AUTO_INDEX=true
   Tests:
     - Test indexing Go repository
     - Test multi-language repository
     - Test error handling (unsupported language)
   Done when: gestalt index command works

** [#C] Optimize query performance and add caching
   Why: SCIP queries may be frequent, need to be fast
   Change:
   - Add in-memory caching for common queries:
     ```go
     type IndexCache struct {
        symbols   *lru.Cache // Symbol definitions
         refs      *lru.Cache // References
         fileSyms  *lru.Cache // File symbols
     }

     func (h *SCIPHandler) FindSymbolsCached(query string) ([]Symbol, error) {
         key := "symbols:" + query
         if cached, ok := h.cache.symbols.Get(key); ok {
             return cached.([]Symbol), nil
         }

         symbols, err := h.index.FindSymbols(query, 20)
         if err != nil {
             return nil, err
         }

         h.cache.symbols.Add(key, symbols)
         return symbols, nil
     }
     ```
   - Add SQLite indexes on commonly queried columns:
     ```sql
     CREATE INDEX idx_symbols_name ON symbols(name);
     CREATE INDEX idx_occurrences_symbol ON occurrences(symbol_id);
     CREATE INDEX idx_documents_path ON documents(path);
     ```
   - Consider full-text search (FTS5) for symbol names:
     ```sql
     CREATE VIRTUAL TABLE symbols_fts USING fts5(symbol_id, name, signature);
     ```
   - Add query batching: if agent queries multiple symbols, batch queries
   - Add metrics: track query times, cache hit rates
   - Profile slow queries, optimize with EXPLAIN QUERY PLAN
   - Consider preloading common symbols on index load
   Tests:
     - Benchmark queries: find symbols, get references
     - Test cache hit rates
     - Profile memory usage with large indexes
   Done when: Queries complete in <100ms for typical codebase

** [#C] Add index freshness checking and auto-reindexing
   Why: Code changes invalidate indexes, need to detect staleness
   Change:
   - Track index generation time in metadata:
     ```go
     type IndexMetadata struct {
         CreatedAt   time.Time
         ProjectRoot string
         Languages   []string
         FilesHashed string // Hash of indexed files for freshness check
     }
     ```
   - On index load, check if files changed since last index:
     ```go
     func (idx *Index) IsFresh() (bool, error) {
         ...
         currentHash := hashSourceFiles(meta.ProjectRoot)

         return currentHash == meta.FilesHashed, nil
     }
     ```
   - Add `scip-api/status` endpoint:
     ```json
     {
       "indexed": true,
       "fresh": false,
     ...
     ```
   - Add config: GESTALT_SCIP_AUTO_REINDEX=true
     - On startup, check freshness
     - If stale (e.g., >24h old), trigger re-index
   - Add optional file watcher integration:
     - Use existing watcher to detect source file changes
     - Queue re-index when significant changes detected
     - Debounce: only re-index after N minutes of inactivity
   - Log index status at startup:
     ```
     INFO SCIP index loaded: 108 documents, 5517 symbols (created 2h ago)
     WARN SCIP index is stale, consider re-indexing
     ```
   Tests:
     - Test freshness detection with modified files
     - Test auto-reindex triggers correctly
   Done when: Stale indexes detected and optionally auto-reindexed

** [#C] Test with real codebases and document best practices
   Why: Need to validate SCIP integration with diverse projects
   Change:
   - Test on Gestalt itself:
     - Index Gestalt Go code
     - Query for internal/terminal symbols
     - Verify all definitions found
     - Test references are accurate
   - Test on multi-language repository:
     - Example: repo with Go backend + TypeScript frontend
     - Run both scip-go and scip-typescript
     - Merge indexes
     - Verify cross-language queries work (if applicable)
   - Test on large codebases (if available):
     - 10k+ LOC Go project
     - Measure indexing time
     - Measure query performance
     - Verify SQLite size reasonable (< 10x source size)
   - Document best practices in README:
     ...
     **Index storage:**
     - Commit `index.db` to repo (optional, depends on size)
     - Or add to .gitignore, regenerate locally
   ...
   - Add troubleshooting guide:
     - Index not loading: check file permissions
     - Indexing fails: check indexer is executable
     - Symbols not found: check language detection
     - Slow queries: check SQLite indexes exist
   Tests: Manual testing, document findings (deferred until indexer binaries are available)
   Done when: SCIP integration validated on real codebases

** [#C] Add reindex recency guard and warning
   Why: Avoid redundant reindexing when the index was just built
   Change:
   - Use index metadata created_at to detect recent indexes
   - If index age < 10 minutes, warn and skip reindex unless forced
     - CLI: `gestalt index --force` bypasses
     - API: add optional `force` flag in request body to bypass
   - Log a warning when reindex is skipped due to recency
   Tests:
     - CLI test: recent metadata skips without --force
     - API test: recent metadata returns warning without force
   Done when: Recent indexes are skipped with a clear warning unless forced

** [#B] Add comprehensive SCIP API tests using internal/scip/index.go data
   Why: Ensure all SCIP endpoints return correct data using a real Gestalt file and
   catch schema/fallback behavior when defn_enclosing_ranges is empty.
   Change:
   - Build a test SCIP SQLite DB from internal/scip/index.go:
     - Use the real file path (internal/scip/index.go).
     - Derive symbol line numbers by scanning the file in the test.
     - Create occurrences/mentions for definitions and references.
   - Exercise scip-api/symbols, scip-api/symbols/:id, scip-api/symbols/:id/references,
     scip-api/files/:path, scip-api/status, and scip-api/index, asserting all relevant
     response fields (ids, names, paths, lines, roles, counts, freshness).
   - Add explicit negative/edge cases:
     - Missing/invalid query params, invalid limits, bad path escapes.
     - Missing symbol/file cases (404/empty results).
     - Rate limiting (429).
     - Index unavailable (503).
     - Reindex conflicts/recent-skip path.
   - Include coverage where defn_enclosing_ranges is empty to verify fallback to
     chunk occurrences for definitions and file symbols.
   Tests:
   - New API tests assert full payloads for success + error responses.
   - Use internal/scip/index.go as the source of truth for expected paths/lines.
   Done when: All SCIP endpoints have broad success/error assertions and fallback
   behavior is covered against internal/scip/index.go.

** [#B] Use system curl/wget for indexer downloads
   Why: Current Go download code appears unreliable; use system tools instead.
   Change:
   - Replace indexer download implementation with exec of `curl` or `wget`
     - Prefer `curl` when available, otherwise fallback to `wget`
     - Ensure exit codes/errors are surfaced with useful messages
     - Preserve existing cache paths and file permissions
   - Keep URL logic unchanged (still using release asset URLs)
   Tests:
     - Unit test: downloader selects curl/wget appropriately
     - Unit test: download failure returns error
   Done when: Indexer downloads succeed via system tooling with coverage

** [#B] Restore Go downloader with versioned asset validation
   Why: System downloader is unreliable; ensure assets match expected version.
   Change:
   - Revert download logic to Go HTTP downloader
   - Ensure indexer asset filename includes the configured version string
   - Surface a clear error when the asset name/version mismatch
   Tests:
     - Unit test: asset name must include version
     - Unit test: download failure returns error
   Done when: Go downloader restored with version-aware asset validation

** [#B] Embed SCIP indexer assets and extract to .gestalt/scip
   Why: Avoid runtime downloads and ensure deterministic indexer availability.
   Change:
   - Add script in script/ to download scip_* indexers into assets/scip/
     - Easy to edit list of URLs per language
     - Download + unpack (if needed) into binary files
     - Write assets/scip/manifest.json with FNV1a 64 hashes
   - Embed assets/scip in gestalt binary
   - Extract assets/scip to .gestalt/scip on startup
     - Skip extraction when hash matches existing file
     - Record/compare FNV1a 64 hash for equality
   - Update indexer lookup path to prefer .gestalt/scip
   Tests:
     - Unit test: manifest extraction skips matching hash
     - Unit test: indexer lookup uses .gestalt/scip
   Done when: Embedded indexers extract on startup with hash checks

** [#C] Consider MCP server adapter (future work)
   Why: May want to expose SCIP to external tools via MCP
   Change: Document design for future implementation
   - MCP (Model Context Protocol) is Anthropic's standard for LLM context providers
   - MCP server exposes:
     - **Resources**: read-only context items (e.g., symbol definitions)
     - **Tools**: actions LLM can invoke (e.g., find_symbol, get_references)
   - Adapter design:
     ```go
     ...
     func (s *MCPServer) ListResources() []Resource {
         // Return list of symbol resources
     }

     func (s *MCPServer) ReadResource(uri string) (string, error) {
         // Return symbol definition as text
     }

     func (s *MCPServer) ListTools() []Tool {
         return []Tool{
             {Name: "find_symbol", Description: "Search for code symbols"},
             {Name: "get_references", Description: "Find symbol references"},
         }
     }

     func (s *MCPServer) CallTool(name string, args map[string]interface{}) (interface{}, error) {
         // Dispatch to SCIP query methods
     }
     ```
   - Run as separate process: `gestalt-mcp-scip`
   - Or embed in Gestalt: `gestalt --mcp-port 8081`
   - External tools (Claude Desktop, Cursor, etc) can connect
   - Benefits:
     - Reusable by other AI tools
     - Standard protocol (no custom API)
     - Ecosystem compatibility
   - Trade-offs:
     - Extra complexity (JSON-RPC server)
     - Authentication/security considerations
     - May not be needed if only Gestalt uses it
   - Decision: No MCP adapter for now; REST API is sufficient for Gestalt agents
   Tests: None (future design only)
   Done when: MCP adapter design documented for future

** [#B] Update documentation and add SCIP LLM prompt file
   Why: LLMs and users need to know SCIP is available; LLMs need a concise prompt for API usage
   Change:
   - Add to README.md:
     ```markdown
     ## Code Intelligence (SCIP)

     Gestalt can index your codebase for intelligent code search:
     ...
     # Index specific path
     gestalt index --path /path/to/repo
     ```

     This enables agents to:
     - Find symbol definitions with documentation
     - Trace function calls and references
     - Understand type relationships
     - Navigate code structure semantically

     Supported languages: Go, TypeScript/JavaScript, Python, Java.

     ### How it works

     1. SCIP indexers scan source code
     2. Generate symbol database (index.db)
     3. Agents query via REST API
     4. Relevant code snippets included in LLM context
     ...
     - `GET scip-api/symbols/:id/references` - Find usages
     - `GET scip-api/files/:path` - List symbols in file
     - `POST scip-api/index` - Trigger re-indexing

     See [SCIP documentation](https://github.com/sourcegraph/scip) for details.
     ```
   - Add concise LLM-facing prompt at `config/prompts/scip.txt`:
     - Detect `.gestalt/scip/index.db` presence
     - Prefer SCIP API queries over filesystem grep for symbols/context
     - Show minimal request patterns and response usage
     - Do not auto-wire into other prompts (manual inclusion only)
   - Update TESTING.md with SCIP testing instructions:
     ```markdown
     ## Testing SCIP Integration

     ```bash
     # Generate test index
     gestalt index --path . --output test-index.db

     # Query API
     curl http://localhost:8080scip-api/symbols?q=Manager
   ...
   Done when: SCIP documented in README and TESTING.md, prompt exists at config/prompts/scip.txt
* DONE [#A] Config extraction refactor with hash verification and version tracking
  Goal: Refactor config/ handling to always extract embedded resources to .gestalt/config/ on startup with FNV-1a 64 hash verification, semantic version compatibility checks, backup on conflicts, and relocate PLAN.org to .gestalt/.
  Notes:
  ...
  - Hash verification: store FNV-1a 64 hashes of embedded files at compile time
...
** [#B] Research and design version.json and manifest formats
   ...
   - Manifest structure: {"path/to/file.json": "fnv1a", ...}
   - Consider: single manifest.json vs separate hash files per resource type
   - Research: Go stdlib/fnv1a for hash generation at build time
   ...
   Decision:
   - version.json location: .gestalt/version.json (written at startup)
   - Required fields: version (semver string), major, minor, patch, built (RFC3339 UTC)
   - Optional fields: git_commit (short SHA), build_host, go_version
   - manifest.json location: config/manifest.json embedded with config FS
   - Manifest format: JSON object of config-relative path -> fnv1a hex (file bytes)
   - Manifest keys use forward slashes, no leading "./"
   - Manifest excludes itself and directories (files only)
   - Example version.json: {"version":"1.2.3","major":1,"minor":2,"patch":3,"built":"2026-01-11T12:34:56Z","git_commit":"abc1234"}
   - Example manifest.json: {"agents/copilot.json":"<fnv1a>","skills/core/SKILL.md":"<fnv1a>"}
   Tests: None (design only)
   Done when: Version and manifest formats designed, documented in this L2

** [#B] Create build-time manifest and version generator
   Why: Need to compute FNV1a 64 hashes and embed version info at compile time
   Change:
   - Create scripts/generate-config-manifest.go or shell script
   - Walk config/agents/, config/prompts/, config/skills/ directories
   - Compute FNV1a 64 hash of each file content
...
** [#B] Embed manifest and version metadata
...
** [#B] Implement hash verification logic
   ...
   - For each file in manifest:
     ...
     3. If exists: compute FNV1a 64 of existing file
...
** [#A] Implement version compatibility checking
...
** [#B] Integrate version tracking and extraction into startup
...
** [#C] Handle backup file cleanup policy
...
** [#B] Relocate PLAN.org to .gestalt/PLAN.org
...
** [#C] Add migration helper for existing installations
...
** [#C] Update --extract-config flag behavior
...
** [#B] Update documentation for new config behavior
...
** [#B] Add config validation on extraction
...
** [#C] Optimize extraction performance
...
** [#B] Expose version info in API and logs
...
** [#B] Update frontend for .gestalt/PLAN.org
...
** [#B] Switch config hash algorithm to FNV-1a 64
   Why: Requested change to use the stdlib FNV-1a 64 hash for manifest and comparisons
   Change:
   - Update config manifest generator to use FNV-1a 64
   - Update extraction hash verification to use FNV-1a 64
   - Refresh tests and documentation to match the new hash algorithm
   Tests: Unit tests for hash verification still pass; manifest generator updated
   Done when: Manifest and extraction use FNV-1a 64 and tests pass

** [#C] Add developer mode for config override
...
** [#B] Add integration tests for config extraction flow
...
** [#B] Document upgrade path and version compatibility
...
** [#C] Add telemetry for config extraction metrics
...
** [#B] Avoid emitting CLI flags for unset config values
   Why: Unset cli_config fields should not override CLI defaults.
   Change:
   - Ensure shell command generation only emits flags for explicitly set values
   - Preserve CLI defaults when config omits a key
   - Align codex/copilot shellgen behavior for missing fields
   Tests: Update shellgen and agent shell tests to cover omitted keys
   Done when: Missing cli_config keys emit no flags and tests pass
** [#B] Update example agent TOML to comment-only defaults
   Why: Example configs should not imply defaults or emit flags unless set.
   Change:
   - Ensure example `cli_config` sections only use commented keys
   - Add comments listing allowed values for enum-like fields
   - Keep active examples minimal (only required fields)
   Tests: None (documentation/examples only)
   Done when: Example TOML files show commented options and no empty values
* DONE [#A] Comprehensive architecture and code quality review
...
** [#A] Review and document current architecture
...
** [#B] Analyze internal/terminal package (850 LOC, core domain)
   ...
   Output: docs/architecture-review.md (section: Package analysis: internal/terminal)

** [#B] Analyze internal/api package (1100+ LOC, HTTP/WS layer)
   ...
   Output: docs/architecture-review.md (section: Package analysis: internal/api)

** [#B] Analyze internal/watcher package (1426 LOC, filesystem monitoring)
   ...
   Output: docs/architecture-review.md (section: Package analysis: internal/watcher)

** [#C] Analyze internal/agent package (400 LOC, agent profiles)
   ...
   Output: docs/architecture-review.md (section: Package analysis: internal/agent)

** [#C] Analyze internal/skill package (350 LOC, Agent Skills)
   ...
   Output: docs/architecture-review.md (section: Package analysis: internal/skill)

** [#C] Analyze internal/logging package (200 LOC, structured logging)
   ...
   Output: docs/architecture-review.md (section: Package analysis: internal/logging)

** [#C] Analyze internal/orchestrator package (empty stub)
   ...
   Output: docs/architecture-review.md (section: Package analysis: internal/orchestrator)

** [#C] Analyze internal/version package (5 LOC, version string)
   ...
   Output: docs/architecture-review.md (section: Package analysis: internal/version)

** [#B] Analyze cmd/gestalt (910 LOC, main entrypoint)
   ...
   Output: docs/architecture-review.md (section: Package analysis: cmd/gestalt)

** [#B] Analyze cmd/gestalt-send (761 LOC, CLI tool)
   ...
   Output: docs/architecture-review.md (section: Package analysis: cmd/gestalt-send)

** [#B] Analyze frontend architecture (4357 LOC, Svelte SPA)
   ...
   Output: docs/architecture-review.md (section: Package analysis: frontend architecture)

** [#B] Analyze frontend/src/lib (store and utility modules)
   ...
   Output: docs/architecture-review.md (section: Package analysis: frontend/src/lib)

** [#B] Analyze frontend components (largest: Terminal 347 LOC, CommandInput 332 LOC)
   ...
   Output: docs/architecture-review.md (section: Package analysis: frontend components)

** [#B] Analyze frontend views (4 main views, thin layer over components)
   ...
   Output: docs/architecture-review.md (section: Package analysis: frontend views)

** [#B] Analyze Go dependencies (4 external, all justified?)
   ...
   Output: docs/architecture-review.md (section: Package analysis: Go dependencies)

** [#C] Analyze frontend dependencies (2 + dev deps)
   ...
   Output: docs/architecture-review.md (section: Package analysis: frontend dependencies)

** [#B] Identify code duplication and extraction opportunities
   ...
   Output: docs/architecture-review.md (section: Cross-cutting analysis: duplication and extraction)

** [#B] Review error handling consistency
   ...
   Output: docs/architecture-review.md (section: Cross-cutting analysis: error handling consistency)

** [#B] Review testing strategy and coverage gaps
   ...
   Output: docs/architecture-review.md (section: Cross-cutting analysis: testing strategy and coverage gaps)

** [#A] Create prioritized improvement roadmap
   ...
   Output: docs/architecture-review.md (section: Prioritized improvement roadmap)

** [#B] Create REFACTOR.org from architecture findings
   Why: Provide a dedicated refactor backlog grouped by implementation theme
   Change:
   - Create REFACTOR.org in org-mode format mirroring PLAN structure
   - Group findings into L1 themes with L2 actionable steps
   - Include large refactors and optional SCIP build tags
   Tests: None (documentation only)
   Done when: REFACTOR.org exists with grouped L1/L2 entries
   Output: REFACTOR.org
...
** [#C] Optional recursive watch and context-based interface
   ...
   Change:
   - Add optional recursive watch mode behind a flag.
   - Accept context in watch registration methods.
   - Document compatibility expectations.
   Tests:
   - Add tests for recursive registration and cancellation.
   Done when: recursion is optional and context cancels watches.
...
* DONE [#A] Wails desktop application packaging
...
* DONE [#B] Fix terminal natural touch scrolling
...
** [#A] Research and analyze why current touch scroll fails
...
** [#B] Choose between Touch Events vs Pointer Events API
...
** [#B] Design pointer-based scroll architecture
   ...
   Architecture notes:
   - Use pointer events on `term.element` with capture + passive false to intercept touch before xterm.
   - Track one active pointerId; ignore additional touches to avoid jumpy multi-touch.
   - Use `term.scrollLines(deltaLines)` to keep xterm buffer state consistent; avoid `scrollTop` writes.
   - Compute pixels-per-line from measured line height when available; fallback to 20px.
   - State machine: idle -> tracking (pointerdown) -> scrolling (threshold exceeded) -> idle (pointerup/cancel).
   - For touch pointers: call preventDefault + stopPropagation in capture phase; for mouse/pen do nothing.
   - Sync scroll state after scrollLines to keep `atBottom` accurate.
   Tests: None (design only)
   Done when: Architecture documented with event flow diagram, edge cases identified

** [#B] Implement pointer event handlers with type detection
...
** [#B] Integrate pointer scroll into attach/detach lifecycle
...
** [#C] Add CSS touch-action: none to xterm element
...
** [#B] Calculate scroll sensitivity (deltaY to scrollLines conversion)
...
** [#B] Add scroll activation threshold to prevent accidental scrolling
...
** [#B] Preserve xterm text selection for mouse
   ...
   - Pointer handlers return early for non-touch pointer types; mouse/pen events reach xterm unchanged.
   - Added test coverage to ensure mouse pointer events do not trigger scrolling.
...
** [#B] Test on real touch devices (phone, tablet)
   ...
   Status: user confirmed touch scrolling works on real devices.
...
** [#C] Add inertial touch scrolling (optional)
   Why: Momentum scrolling can feel more natural on touch devices for long terminal output.
   Change:
   - Track touch scroll velocity during pointermove (deltaY / deltaTime).
   - On pointerup, if velocity exceeds a small threshold, start inertial scroll loop.
   - Use requestAnimationFrame to apply decaying velocity (friction) over time.
   - Stop inertia when velocity falls below threshold or user touches again.
   - Use term.scrollLines with accumulated delta, sync scroll state on each step.
   - Keep mouse/pen behavior unchanged; inertia applies only to touch pointer type.
   - Add safety: cap max velocity to prevent large jumps on noisy events.
   Tests: Manual testing on touch devices; unit test for inertial state/cleanup if feasible.
   Done when: Fast swipe continues scroll briefly with smooth deceleration.

** [#C] Add visual feedback for touch scroll (optional)
   ...
   Decision: defer; no additional visual feedback requested after touch scroll fix.
...
** [#B] Add touch scroll integration tests (automated)
   ...
   Notes:
   - Added pointer scroll tests in `frontend/tests/terminalStore.test.js` covering touch vs mouse,
     thresholds, sensitivity, and inertial scroll behavior.
...
** [#B] Update documentation for touch scroll feature
   ...
   Notes:
   - Documented touch scrolling and momentum in README.md.
   - Added LLM context in AGENTS.md and manual checklist in TESTING.md.
   Tests: None (documentation only)
   Done when: Touch scroll documented in user and developer docs

** [#C] Tune inertial scroll feel (duration + additive momentum)
   Why: Improve momentum duration, deceleration feel, and allow successive swipes to add speed.
   Change:
   - Increase inertia duration with slower decay and lower cutoff threshold.
   - Accumulate fractional scroll lines to avoid “stepy” deceleration.
   - Preserve/add velocity when a new swipe happens during momentum for natural acceleration.
   - Clamp velocity to avoid excessive jumps.
   - Adjust smoothing to capture faster flicks without jitter.
   Tests: Update/extend touch inertia unit tests as needed; confirm on device.
   Done when: Momentum lasts longer, decelerates smoothly, and successive swipes add speed.

** [#B] Preserve scrollbar drag behavior on touch
   Why: Touching the scrollbar should drag the viewport rather than triggering touch scroll handlers.
   Change:
   - Detect touch interaction on `.xterm-scrollbar`/`.xterm-viewport` scrollbar area.
   - If touch starts on scrollbar, bypass custom touch scrolling and allow native drag.
   - Ensure pointer capture is not set for scrollbar touches.
   - Keep mouse selection behavior unchanged.
   - Confirm on device that dragging the scrollbar works and touch scrolling still works elsewhere.
   Tests: Manual test on touch devices; add unit coverage if feasible.
   Done when: Touching the scrollbar uses native scrollbar drag without breaking touch scrolling.

** [#A] Validate with user testing before marking done
...
* DONE [#B] Enhanced input UX with multiline input box and command history
...
** DONE [#B] Fix skill prompt injection order and content
...
** DONE [#B] Update available_skills metadata format and stop terminal printing
...
* DONE [#B] Fix codex CLI override syntax and log generated commands
  Effort: Small
  Goal: Ensure codex CLI config overrides use the expected syntax and make the
  generated shell commands visible at debug level for all agent configs.
  Notes: Codex CLI now rejects "key:value" overrides; we should emit "key=value"
  and log the rendered command for each agent.
** DONE [#B] Switch codex shellgen to key=value and update tests
   Effort: Small
   Why: Codex CLI errors on overrides without '=' (e.g., approval_policy:never).
   Change: Update codex shellgen to emit key=value; update tests expecting
   key:value output.
   Tests: internal/agent/shellgen/codex_test.go, internal/agent/shell_test.go,
   internal/agent/integration_test.go, internal/terminal/manager_test.go.
   Done when: Tests expect and pass with key=value overrides.
** DONE [#C] Log rendered CLI commands at debug for all agent configs
   Effort: Small
   Why: Help debug CLI config rendering without waiting for session start.
   Change: Log the generated shell command during agent load when cli_config is
   present (and not empty).
   Tests: If needed, add a unit test in internal/agent/loader_test.go to assert
   debug log output.
   Done when: Debug logs show each agent_id + rendered shell command.

* DONE [#A] Terminal session refactor and logging consolidation
  Goal: Reduce terminal/session complexity while preserving behavior and tests.
** [#B] Extract AsyncFileLogger for session and input logs
   Effort: Medium
   Change: Create shared async file logger for SessionLogger/InputLogger.
   Tests: Update persistence tests to cover new shared logger.
** [#B] Split Session into focused sub-structs
   Effort: Medium
   Change: Introduce SessionMeta/SessionIO/SessionWorkflow with a wrapper type.
   Tests: Adjust session tests to cover new struct boundaries.

* DONE [#B] WebSocket and event stream consolidation
  Goal: Reduce duplicated WS handling in backend and frontend.
** [#B] Create backend WS helper
   Effort: Small
   Change: Extract upgrade + write loop helper used by WS handlers.
   Tests: Add handler tests for close/error handling.
** [#B] Create frontend wsStore helper
   Effort: Small
   Change: Introduce shared wsStore utility for event stores and migrate them.
   Tests: Update store tests to exercise the shared helper.

* DONE [#B] Frontend state/store simplification
  Goal: Make large UI modules smaller and more testable.
** [#B] Split terminalStore into focused modules
   Effort: Medium
   Change: Separate xterm setup, ws connection, and input handling modules.
   Tests: Keep terminalStore tests passing with new module boundaries.
** [#B] Move Dashboard data orchestration into a store
   Effort: Medium
   Change: Create a dashboardStore for agents/skills/logs fetching and state.
   Tests: Add store tests for loading/error behavior.

* WIP [#A] Diagnose and fix high CPU usage in idle gestalt
  Effort: 1-2 days
  Goal: Identify and eliminate idle CPU usage when Gestalt is not actively used.
  Notes:
  - Focus on server + watcher/event loops + WS write loops + frontend polling
  - Prefer minimal changes; fix root causes instead of adding throttles unless needed
  - Use profiling to locate hot paths before changing behavior

** DONE [#A] Baseline idle CPU usage and capture profiles
   Effort: 2-4 hours
   Change:
   - Reproduce idle state (no active terminals, no user interactions)
   - Capture CPU profiles and goroutine profiles
   - Record top hot paths with file/line notes
   Notes:
   - 2026-01-25: 30s CPU profile (pprof.gestalt.samples.cpu.003.pb.gz) shows 63% cumulative in
     internal/api.(*RestHandler).handleLogs -> internal/otel.scanOTelFile -> encoding/json.Unmarshal.
   - Hot functions are json.checkValid/unquote/stateInString plus GC/malloc, indicating heavy JSON
     decoding churn from log reads.
   - This points to frequent /api/logs (OTel log) polling while idle.
   Tests:
   - N/A (profiling-only step)
   Done when:
   - Profile artifacts collected and summarized in this L2 notes

** DONE [#A] Identify root cause and implement fix
   Effort: 4-6 hours
   Change:
   - Update the offending loop/timer/polling logic to be event-driven or throttled
   - Ensure no regression in terminal/output streaming or watcher updates
   - Add a small guard against tight loops where appropriate
   Plan:
   - Switch UI log consumption to the /ws/logs stream (event-driven) instead of polling /api/logs or /api/otel/logs.
   - Add initial log snapshot on /ws/logs connect (send logger buffer before streaming) so UI can
     populate without a REST fetch.
   - Keep log level filtering via websocket message ({"level":"error"}).
   Tests:
   - Add/adjust unit tests to cover the new behavior
   - Run backend tests
   Done when:
   - Idle CPU usage reduced and profile confirms hot path resolved
   - Tests pass

** DONE [#B] Add regression coverage for idle CPU behavior
   Effort: 2-4 hours
   Change:
   - Add tests that ensure loops back off when idle (time-based or event-driven)
   - Document the root cause and fix in AGENTS.md
   Tests:
   - Run backend and relevant frontend tests if touched
   Done when:
   - Tests pass and documentation updated

* DONE [#B] Apply prompt templating to all prompt files
  Effort: 2-4 hours
  Goal: Simplify prompt templating by applying directives to any prompt file in the prompts directory, regardless of extension.
  Context:
  - Current behavior: only `.tmpl` files process `{{include ...}}` and `{{port ...}}` directives.
  - Desired behavior: `.tmpl`, `.md`, and `.txt` (and any other prompt file) should process directives.
  - Keep directive rules: directives must be on their own line; unknown includes/ports are skipped silently.
  Constraints:
  - No new dependencies.
  - Minimize behavior changes outside prompt rendering.

** [#B] Update prompt parser to process directives for all prompt files
   Effort: 1-2 hours
   Change:
   - In `internal/prompt/parser.go`, remove the extension gate that skips templating for non-`.tmpl` files.
   - Continue to parse directives line-by-line for every prompt file.
   - Keep existing safeguards: depth limit, cycle detection, text-only includes, silent skips.
   Tests:
   - Update/add unit tests in `internal/prompt/parser_test.go` to cover `.md` and `.txt` directives.
   - Run: `GOMODCACHE=/tmp/gomodcache GOPATH=/tmp/gopath GOCACHE=/tmp/gocache /usr/local/go/bin/go test ./internal/prompt -count=1`
   Done when:
   - `{{include ...}}` and `{{port ...}}` work in `.txt` and `.md` prompts.

** [#B] Add regression tests for directive behavior in non-.tmpl prompts
   Effort: 1 hour
   Change:
   - Add focused tests showing:
     - `.txt` prompt with `{{include ...}}` renders included content.
     - `.md` prompt with `{{port backend}}` renders a numeric port.
     - Directives still require their own line.
   Tests:
   - Run backend prompt tests and full backend suite.
   Done when:
   - Tests fail on old behavior and pass on new behavior.

** [#B] Document simplified templating rules for humans and agents
   Effort: 30-60 minutes
   Change:
   - Update `config/prompts/README.md` to state that templating applies to all prompt files.
   - Update `AGENTS.md` prompt templating section to match.
   Tests:
   - N/A (docs only)
   Done when:
