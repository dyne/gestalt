...
* DONE [#A] SCIP code intelligence integration for LLM context enhancement
  ...
  **Key Components:**
  ...
  4. **REST Endpoints**: `/api/scip/symbols`, `/api/scip/references`, etc
...
** [#B] Add REST API endpoints for SCIP queries
   ...
   - Register routes in internal/api/routes.go:
     ...
     if err == nil {
         router.HandleFunc("/api/scip/symbols", scipHandler.FindSymbols)
         router.HandleFunc("/api/scip/symbols/{id}", scipHandler.GetSymbol)
         router.HandleFunc("/api/scip/symbols/{id}/references", scipHandler.GetReferences)
         router.HandleFunc("/api/scip/files/{path}", scipHandler.GetFileSymbols)
         router.HandleFunc("/api/scip/index", scipHandler.ReIndex).Methods("POST")
     }
     ```
...
** [#B] Create agent prompt/skill documenting SCIP API usage
   ...
   - Create .gestalt/config/skills/code-intelligence/SKILL.md:
     ...
     **Understanding a function:**
     1. Search for function: `/api/scip/symbols?q=ProcessOrder`
     2. Get definition: `/api/scip/symbols/<id>` (includes documentation)
     3. Find callers: `/api/scip/symbols/<id>/references`
     4. Read source at definition location

     **Finding related code:**
     1. Search for type: `/api/scip/symbols?q=User`
     2. Get type definition and fields
     3. Find methods on type (search `User.`)
     4. Find usage patterns via references

     **Exploring a module:**
     1. List symbols in file: `/api/scip/files/internal/user/user.go`
   ...
   - Alternatively: Add to default agent prompt template:
     ...
     Available endpoints:
     - GET /api/scip/symbols?q=<query> - Search symbols
     - GET /api/scip/symbols/<id> - Get definition
     - GET /api/scip/symbols/<id>/references - Find usages
     - GET /api/scip/files/<path> - List symbols in file
   ...
   - Add examples to AGENTS.md for LLMs:
     ...
     - REST API: `/api/scip/symbols`, `/api/scip/symbols/:id/references`
     ...
     - Example: "Find all callers of ProcessOrder function"
       1. Search: GET /api/scip/symbols?q=ProcessOrder
       2. Get ID from results
       3. Query: GET /api/scip/symbols/<id>/references
...
** [#B] Add comprehensive SCIP API tests using internal/scip/index.go data
   ...
   - Exercise /api/scip/symbols, /api/scip/symbols/:id, /api/scip/symbols/:id/references,
     /api/scip/files/:path, /api/scip/status, and /api/scip/index, asserting all relevant
...
** [#B] Update documentation and add SCIP LLM prompt file
   ...
   - Add to README.md:
     ...
     - `GET /api/scip/symbols/:id/references` - Find usages
     - `GET /api/scip/files/:path` - List symbols in file
     - `POST /api/scip/index` - Trigger re-indexing
   ...
   - Update TESTING.md with SCIP testing instructions:
     ...
     # Query API
     curl http://localhost:8080/api/scip/symbols?q=Manager
