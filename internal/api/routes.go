...
func RegisterRoutes(mux *http.ServeMux, manager *terminal.Manager, authToken string, statusConfig StatusConfig, scipIndexPath string, scipAutoReindex bool, staticDir string, frontendFS fs.FS, logger *logging.Logger, eventBus *event.Bus[watcher.Event]) {
	...
	if scipIndexPath != "" {
		...
		if err != nil {
			...
			mux.Handle("/api/scip/status", wrap("/api/scip/status", "config", "read", restHandler(authToken, scipHandler.Status)))
			mux.Handle("/api/scip/symbols", wrap("/api/scip/symbols", "config", "read", restHandler(authToken, scipHandler.FindSymbols)))
			mux.Handle("/api/scip/symbols/", wrap("/api/scip/symbols/:id", "config", "read", restHandler(authToken, scipHandler.HandleSymbol)))
			mux.Handle("/api/scip/files/", wrap("/api/scip/files/:path", "config", "read", restHandler(authToken, scipHandler.GetFileSymbols)))
			mux.Handle("/api/scip/index", wrap("/api/scip/index", "config", "update", restHandler(authToken, scipHandler.ReIndex)))
		}
	}
